...
async def clear_collections():
    ...
    cols = [
        ...
        "design_requests",
        # FASE I — dokumen inspeksi (SPK QC) menempel pada PO, tugas penerimaan,
        # ROLL, retur, dan sample acuan yang SEMUANYA ikut direset di atas. Kalau
        # dibiarkan hidup ia langsung menjadi yatim: `INV-REF-01` memerah ("dokumen
        # turunan tanpa induk hidup") dan `audit_doc_refs` melaporkan tautan
        # menggantung ke `purchase_order` yang sudah tidak ada. Ini BUKAN kesalahan
        # kode FASE I — ini pelajaran yang sama dengan `design_requests` di atas:
        # dokumen transaksional yang tidak ikut direset akan menuduh fase lain.
        "inspections",
        ...
        "process_stages",
        # FASE S — master JENIS SAMPLING. Alasan reset sama dengan lini & tahapan:
        # tanpa ini `seed_sample_types()` menambah baris kembar setiap seed ulang,
        # dan `_assert_key_free` akan menolak jenis baru yang dibuat pemilik lewat
        # layar karena kodenya dianggap sudah terpakai.
        "sample_types",
        # FASE I — master ALASAN KELUHAN RETUR. Alasan reset sama dengan lini ·
        # tahapan · jenis sampling: tanpa ini `seed_complaint_reasons()` menambah
        # baris kembar setiap seed ulang, dan `_assert_key_free` akan menolak kode
        # keluhan baru yang dibuat pemilik lewat layar.
        "complaint_reasons",
    ]
    for col in cols:
...
async def seed_users():
    ...
    await db.users.insert_many([
        ...
        # ── FB-02 (2026-09-02) — PERAN KE-8: SOPIR armada sendiri (modul Logistik).
        {
            "id": "user_driver_01", "name": "Joko Susilo",
            "email": "driver@kainnusantara.id", "phone": "081200000014",
            "role": "driver", "password_hash": hash_password("demo12345"), "status": "active",
            "home_entity_id": "ent_ksc", "allowed_entity_ids": ["ent_ksc"],
            "created_at": ago(days=30)
        },
        # ── FASE P (2026-08-21) — MD BERPAGAR LINI untuk PAPAN PO. Pemilik:
        # "Sebagai MD printing, saya tidak melihat PO woven — bukan urusan saya."
        # Akun ini WAJIB ada di data demo dengan alasan yang sama seperti
        # `dewi.printing@` (FASE L): pagar lini pada PAPAN PO hanya bisa diuji
        # LEWAT LAYAR kalau ada akun yang benar-benar berpagar DAN berhak melihat
        # PO. `dewi.printing@` berperan `sales`, yang memang TIDAK punya izin
        # `purchase_order.view`, jadi ia tak bisa membuktikan apa pun di papan ini.
        # Perannya `manager` karena papan PO adalah pekerjaan MD/manajer.
        {
            "id": "user_manager_03", "name": "Fajar Nugroho",
            "email": "manager.printing@kainnusantara.id", "phone": "081200000013",
            "role": "manager", "password_hash": hash_password("demo12345"),
            "status": "active",
            "home_entity_id": "ent_ksc", "allowed_entity_ids": ["ent_ksc"],
            "allowed_line_codes": ["printing"],
    ...
    print("✅ Users seeded (with entity assignment) — 13 akun · 7 peran "
          "(admin · manager×3 · sales_admin · finance · sales×4 · warehouse×2 · designer) "
          "· 2 akun berpagar lini printing (FASE L: sales · FASE P: manajer papan PO)")
...
# MD-07 — nama versi PABRIK/supplier untuk warna yang sama (contoh demo; dicari juga).
COLOR_FACTORY_NAMES = {
    "KN-BLU-01": "Indigo 19-3921 (Pabrik Cirebon)",
    "KN-BLU-02": "Navy Blazer 19-3923",
    "KN-BRN-01": "Sogan Tua (Solo Weave)",
    "KN-RED-01": "Maroon 19-1655",
    "KN-GRN-03": "Teal Green 18-5128",
}
...
async def seed_color_library():
    ...
    for code, name, hexv, system, family in COLOR_LIBRARY_SEED:
        docs.append({
            ...
            "factory_name": COLOR_FACTORY_NAMES.get(code, ""),
...
async def seed_product_lines():
    ...
    rows = [
        {"id": "pline_woven", "code": "woven", "name": "Woven (Tenun)", "sort": 1,
         ...
         # FASE P — kode di sini adalah kode TAHAP PROSES (`process_stages`), bukan
         # tahap BAHAN (`yarn`/`grey`/`finished`). Lihat komentar di
         # `domain_registry.PRODUCT_LINES`; dijaga gate `INV-STAGE-01`.
         "stage_sequence": ["benang", "tenun", "celup", "inspect"],
         # FASE S (keputusan pemilik #3): woven & knit = labdip + handfeel.
        ...
        {"id": "pline_printing", "code": "printing", "name": "Printing", "sort": 3,
         ...
         # FASE S (keputusan pemilik #3): printing = proofing (labdip menyusul bila
         # warna dasarnya juga perlu dicocokkan).
...
# FASE S — field master jenis sampling (sama dengan
# `entity_master_service.MASTERS["sample-types"]`).
SAMPLE_TYPE_FIELDS = (
    "code", "name", "applies_to_lines", "seq", "active", "notes",
    "requires_design", "measurement_fields",
)


async def seed_sample_types():
    """FASE S — master JENIS SAMPLING (berlapis: baris GLOBAL `entity_id="all"`).

    Nilainya diambil LANGSUNG dari benih `domain_registry.SAMPLE_TYPES` — bukan
    disalin ke sini, alasan yang sama dengan FASE L & T: dua daftar akan menyimpang
    dalam beberapa sesi dan tidak ada yang tahu mana yang benar.
    `scripts/migrate_sample_types.py` membaca sumber yang sama untuk basis data lama.

    `bulk_sample` di-seed dengan `active=False` (keputusan pemilik #4): ia TIDAK
    dihapus dari enum supaya dokumen lama tetap punya labelnya, tetapi hilang dari
    semua pilihan.
    """
    import domain_registry as _dr
    rows = []
    for seed in _dr.enum_items("sample_type"):
        code = str(seed.get("code") or seed.get("value") or "").strip().lower()
        row = {k: seed.get(k) for k in SAMPLE_TYPE_FIELDS if k in seed}
        row["code"] = code
        row["name"] = seed.get("name") or seed.get("label") or code
        row.setdefault("applies_to_lines", [])
        row.setdefault("measurement_fields", [])
        row.setdefault("requires_design", False)
        row.setdefault("notes", seed.get("description", "") or "")
        rows.append({**row, "id": f"stype_{code}", "entity_id": "all",
                     "active": seed.get("active", True) is not False,
                     "created_by": "Seed", "created_at": ago(days=200),
                     "updated_at": ago(days=200)})
    await db.sample_types.insert_many(rows)
    aktif = [r["code"] for r in rows if r["active"]]
    mati = [r["code"] for r in rows if not r["active"]]
    print(f"✅ Sample Types seeded ({len(rows)} jenis GLOBAL — aktif: "
          f"{' · '.join(aktif)}" + (f" · nonaktif: {' · '.join(mati)}" if mati else "")
          + ") — FASE S")


# FASE I — field master alasan keluhan retur (sama dengan
# `entity_master_service.MASTERS["complaint-reasons"]`).
COMPLAINT_REASON_FIELDS = (
    "code", "name", "applies_to_lines", "seq", "active", "notes",
    "needs_inspection", "claimable",
)


async def seed_complaint_reasons():
    """FASE I — master ALASAN KELUHAN RETUR (berlapis: baris GLOBAL `entity_id="all"`).

    Nilainya diambil LANGSUNG dari benih `domain_registry.COMPLAINT_REASONS` — bukan
    disalin ke sini, alasan yang sama dengan FASE L · T · S: dua daftar akan
    menyimpang dalam beberapa sesi dan tidak ada yang tahu mana yang benar.
    """
    import domain_registry as _dr
    rows = []
    for seed in _dr.enum_items("complaint_reason"):
        code = str(seed.get("code") or seed.get("value") or "").strip().lower()
        row = {k: seed.get(k) for k in COMPLAINT_REASON_FIELDS if k in seed}
        row["code"] = code
        row["name"] = seed.get("name") or seed.get("label") or code
        row.setdefault("applies_to_lines", [])
        row.setdefault("needs_inspection", False)
        row.setdefault("claimable", False)
        row.setdefault("notes", seed.get("description", "") or "")
        rows.append({**row, "id": f"creas_{code}", "entity_id": "all",
                     "active": seed.get("active", True) is not False,
                     "created_by": "Seed", "created_at": ago(days=200),
                     "updated_at": ago(days=200)})
    await db.complaint_reasons.insert_many(rows)
    wajib = [r["code"] for r in rows if r.get("needs_inspection")]
    print(f"✅ Complaint Reasons seeded ({len(rows)} alasan GLOBAL — wajib diperiksa "
          f"fisik: {' · '.join(wajib)}) — FASE I")
...
async def seed_p0_so_pr_po_chain():
    """P-0 (prasyarat FASE P) — rantai NYATA **SO → PR (`so_repeat`) → PO**.

    KENAPA DATA INI HARUS ADA
    -------------------------
    Papan PO (FASE P) memuat kolom **Nama Sales**. Nama itu tidak diketik: ia
    dirunut `PO → PR → SO.sales_name`. Sebelum langkah ini seluruh data demo
    **tidak punya satu pun** rantai seperti itu (terukur: 0 dari 14 PO menyimpan
    `pr_id`; `PR.source` demo hanya `manual`/`reorder`), sehingga:
      · papan PO hanya bisa memperlihatkan kolom Nama Sales yang KOSONG di semua
        baris — pemilik akan menyimpulkan fiturnya rusak, padahal datanya tak ada;
      · agen uji tidak punya apa pun untuk diklik saat memverifikasi P-0/FASE P.

    KENAPA LEWAT LAYANAN NYATA, BUKAN SUNTIK DOKUMEN
    ------------------------------------------------
    Rantai ini dibuat lewat **pintu yang sama dengan tombol pengguna**:
    `restock_service.request_repeat_restock()` (tombol "Ulangi pesanan" di layar
    Pesanan) → `pr_sourcing_service.realize_to_po()` (tombol "Realisasi ke PO" di
    layar PR). Dampaknya penting: kalau kelak salah satu jalur berubah dan
    berhenti menulis asal dokumen, **data demo ikut berubah** dan gate/POC akan
    memerah. Kalau dokumen disuntik mentah, demo akan tetap tampak sehat sementara
    aplikasinya sudah rusak — kelas jebakan "hijau tapi hampa" (§11 risiko #11).

    Yang sengaja TIDAK dilakukan: **membackfill 14 PO lama** (keputusan pemilik
    2026-08-21: "biarkan kosong"). PO pembelian rutin memang tidak punya sales;
    sel kosong yang jujur lebih baik daripada nama yang dikarang.
    """
    import sys as _sys
    _sys.path.insert(0, str(Path(__file__).resolve().parent / "backend"))
    from services import po_board_service as _board
    from services import pr_sourcing_service as _srcng
    from services import restock_service as _restock

    def _in_days(n: int) -> str:
        return (datetime.now(timezone.utc) + timedelta(days=n)).strftime("%Y-%m-%d")

    # (SO asal, produk, qty, supplier, gudang, hari ETA, progres tahap) — produk
    # SENGAJA dipilih dari lini yang berbeda supaya papan PO per lini punya isi di
    # lebih dari satu tab, dan dua sales BERBEDA supaya kolom Nama Sales terbukti
    # mengikuti SO-nya (bukan mengikuti pembuat PO — yang keduanya sama-sama admin).
    #
    # `stages` = progres tahap FASE P. Ditulis lewat `po_board_service.set_stage()`,
    # yaitu PINTU YANG SAMA dengan tombol di layar — bukan disuntik ke dokumen.
    # Kalau aturan papan berubah (mis. tahap turunan bertambah), data demo ikut
    # berubah dan POC/gate akan memerah; suntikan mentah akan tetap tampak sehat.
    # Penanda tahap dipasang oleh **manajer lini printing** untuk PO printing
    # (akun berpagar lini) dan manajer umum untuk PO woven — sehingga jejak "siapa
    # & kapan" di papan masuk akal saat dibuka pemilik.
    CHAINS = (
        ("so_007", "prod_jumputan_palembang", 300.0, "Cirebon Craft", "wh_bandung", 12,
         (("proofing", "done", "Proofing disetujui pelanggan"),
          ("pfp", "done", "Kain PFP siap"),
          ("screen", "in_progress", "Kasa sedang dibuat mitra"))),
        ("so_008", "prod_batik_mega", 250.0, "Cirebon Craft", "wh_jakarta", 20,
         (("proofing", "done", "Proofing ACC"),)),
        ("so_005", "prod_tenun_ikat", 180.0, "NTT Weaving Co", "wh_surabaya", 16,
         (("benang", "done", "Benang diterima mitra tenun"),
          ("tenun", "in_progress", "Tenun jalan di mesin 3"))),
    )
    STAGE_ACTOR = {
        "printing": {"id": "user_manager_03", "name": "Fajar Nugroho",
                     "allowed_line_codes": ["printing"], "role": "manager"},
        "woven": {"id": "user_manager_01", "name": "Dewi Rahayu",
                  "allowed_line_codes": [], "role": "manager"},
    }
    made = 0
    for so_id, product_id, qty, supplier_name, wh_id, eta_days, stages in CHAINS:
        so = await db.sales_orders.find_one({"id": so_id}, {"_id": 0})
        prod = await db.products.find_one({"id": product_id}, {"_id": 0})
        sup = await db.suppliers.find_one({"name": supplier_name}, {"_id": 0, "id": 1})
        if not (so and prod and sup):
            print(f"  [warn] rantai P-0 dilewati (SO/produk/supplier tak ada): {so_id}")
            continue
        # Pemohon = SALES pemilik pesanan itu (bukan admin) — supaya jejak "siapa
        # meminta" masuk akal saat dibuka di layar.
        sales_user = await db.users.find_one({"id": so.get("created_by") or ""},
                                             {"_id": 0, "id": 1, "name": 1}) or {}
        actor_sales = {"id": sales_user.get("id", ""),
                       "name": sales_user.get("name") or so.get("sales_name") or "Sales"}
        try:
            res = await _restock.request_repeat_restock(
                so_id,
                [{"product_id": product_id, "quantity": qty,
                  "unit": prod.get("base_unit", "yard"),
                  "note": f"Ulangi pesanan {so.get('number', '')}"}],
                actor_sales,
                reason=(f"Ulangi pesanan {so.get('number', '')} — "
                        f"{so.get('customer_name', '')}"),
                warehouse_id=wh_id, needed_by_date=_in_days(eta_days), submit_now=True)
            pr = res["pr"]
            if pr.get("status") != "approved":       # nilai di atas ambang → butuh orang
                from services import purchase_requisition_service as _prs
                pr = await _prs.approve_requisition(
                    pr["id"], {"id": "user_manager_01", "name": "Dewi Rahayu",
                               "role": "manager"},
                    notes="Disetujui (data demo rantai P-0)")
            out = await _srcng.realize_to_po(
                pr["id"], supplier_id=sup["id"],
                actor={"id": "user_admin_01", "name": "Budi Santoso"},
                warehouse_id=wh_id, expected_delivery_date=_in_days(eta_days),
                notes=f"Realisasi {pr.get('number', '')} (rantai demo P-0)")
            po = out["po"]
            made += 1
            # FASE P — progres tahap lewat layanan papan (pintu yang sama dengan layar).
            line = (po.get("line_codes") or ["woven"])[0]
            actor_stage = STAGE_ACTOR.get(line, STAGE_ACTOR["woven"])
            for code, status, note in stages:
                try:
                    await _board.set_stage(po["id"], stage_code=code, status=status,
                                           note=note, actor=actor_stage)
                except Exception as _se:  # noqa: BLE001
                    print(f"  [warn] tahap {code} untuk {po.get('po_number')} dilewati: {_se}")
            print(f"   [P-0] {so.get('number')} → {pr.get('number')} → "
                  f"{po.get('po_number')} · Nama Sales dirunut: "
                  f"{po.get('sales_name') or '(kosong)'} · lini {line} · "
                  f"{len(stages)} tahap ditandai")
        except Exception as _e:  # noqa: BLE001
            print(f"  [warn] rantai P-0 {so_id} gagal: {type(_e).__name__}: {_e}")
    print(f"✅ P-0 — rantai SO→PR(so_repeat)→PO dibuat ({made} rantai, lewat layanan nyata)")
    # FASE P — pastikan SELURUH PO (termasuk PO demo lama & kontrak payung) punya
    # field papan `stage_progress`. Lewat PINTU MIGRASI yang sama dengan basis data
    # lama supaya kedua basis data punya arti identik (pola yang sama dengan
    # `line_scope.backfill` di akhir seed).
    try:
        _mb = await _board.ensure_stage_field(db)
        print(f"✅ FASE P — field papan `stage_progress` ada di {_mb['total']} PO "
              f"({_mb['written']} diisi migrasi · {_mb['with_progress']} sudah berprogres)")
    except Exception as _e:  # noqa: BLE001
        print(f"  [warn] migrasi stage_progress dilewati: {_e}")
    return made
...
async def seed_all(db_instance=None):
    ...
    await seed_sample_types()           # FASE S — master jenis sampling (labdip·handfeel·proofing)
    await seed_complaint_reasons()      # FASE I — master alasan keluhan retur
    await seed_products()
    ...
    await seed_budgets()
    # P-0 (prasyarat FASE P) — rantai NYATA SO → PR(`so_repeat`) → PO lewat layanan
    # yang sama dengan tombol pengguna. Posisinya di SINI bukan di dekat
    # `seed_requisitions` karena `realize_to_po` menegakkan **kontrol anggaran**
    # (`budget_service.enforce_po_budget`) yang membaca **bagan akun** `1-1300`:
    # akun itu baru lahir dari `gl_service.seed_default_coa()` beberapa baris di
    # atas. Dijalankan lebih awal, rantainya gagal dengan
    # "Akun COA '1-1300' tidak ditemukan" — dan seed tetap "hijau" karena
    # kegagalannya hanya di-warn. Kelas jebakan yang sama dengan koleksi fondasi
    # (§.restore_env.sh): urutan restart/seed menentukan ada-tidaknya data.
    try:
        await seed_p0_so_pr_po_chain()
    except Exception as _e:  # noqa: BLE001
        print(f"  [warn] seed_p0_so_pr_po_chain dilewati: {_e}")
    ...
    # FASE I — SPK Inspeksi & QC demo + perjalanan retur (lewat LAYANAN produksi,
    # bukan insert mentah — §3.1 titik 15: fitur "hijau tapi hampa" adalah temuan
    # berulang di repo ini).
    try:
        summary["inspections"] = await seed_inspections()
    except Exception as _e:  # noqa: BLE001
        print(f"  [warn] seed_inspections dilewati: {_e}")
...
async def seed_inspections():
    """FASE I — **SPK INSPEKSI & QC** demo + perjalanan retur (lembar pemilik).

    Dibuat lewat **layanan produksi** (`inspection_service`) supaya benar-benar
    melewati aturannya: nomor `<ENT>/INS-#####`, baris dibangun dari roll/barang
    retur yang NYATA, poin cacat & grade dihitung mesin 4-point yang sudah ada
    (`qc_inspection_service` → `grade_service`), tahanan warna mengikuti kebijakan
    Pusat Pengaturan, dan tautan dokumen dua arah.

    Empat keadaan supaya layar tidak kosong DAN antrean keputusan punya angka:
      (1) **draf** — SPK penerimaan PO yang baru lahir otomatis (user story I.1:
          kepala gudang hanya menugaskan, tidak membuat dari nol);
      (2) **sedang diperiksa** dengan satu roll ber-cacat 24 poin → grade turun
          (user story I.2: grade muncul otomatis dari poin);
      (3) **DITAHAN** karena warna beda dari sample yang di-ACC — inilah baris yang
          mengisi antrean `inspection_hold` & KPI beranda manajer (user story I.3);
      (4) **selesai** dengan keputusan → `sales_returns.inspect_done_at` terisi
          sehingga garis waktu retur lengkap (user story I.5).

    Kenapa tidak `insert_one` mentah: dokumen inspeksi yang lahir tanpa melewati
    layanan akan LOLOS dari gate `INV-QC-02` (poin dokumen == poin roll) dan dari
    pagar putaway — data demo yang tampak sehat padahal tidak pernah membuktikan
    apa pun.
    """
    import sys as _sys
    ...
    from services import return_service as ret_svc

    admin = await db.users.find_one({"id": "user_admin_01"}, {"_id": 0})
    actor_admin = {"id": admin["id"], "name": admin["name"], "role": "admin"}
    petugas = await db.users.find_one({"role": "warehouse", "home_entity_id": "ent_ksc"},
                                      {"_id": 0}) or {}
    made = 0

    # ── (1) SPK penerimaan PO — LAHIR OTOMATIS untuk setiap tugas di antrean QC ──
    # Data demo memasang tugas `qc_pending` langsung (bukan lewat alur terima), jadi
    # tanpa langkah ini janji "SPK-nya sudah ada" tidak terbukti di data demo — dan
    # `GET /api/inspections/queue/qc-tasks` (jaring pengaman) justru menampilkannya
    # sebagai pekerjaan yang terlewat.
    tasks = await db.wms_tasks.find({"flow_type": "inbound", "status": "qc_pending"},
                                    {"_id": 0}).to_list(50)
    po_docs = []
    for t in tasks:
        try:
            po_docs.append(await ins.ensure_for_qc_task(t, actor_admin))
            made += 1
        except Exception as exc:  # noqa: BLE001
            print(f"  [warn] SPK inspeksi untuk tugas {t.get('id')} dilewati: {exc}")

    # ── (2)+(3) satu SPK KSC dikerjakan sampai ada roll DITAHAN ─────────────────
    ksc = next((d for d in po_docs
                if d.get("entity_id") == "ent_ksc" and (d.get("lines") or [])), None)
    if ksc and petugas:
        actor_petugas = {"id": petugas["id"], "name": petugas["name"], "role": "warehouse"}
        ksc = await ins.assign(ksc["id"], {"assigned_to": petugas["id"],
                                           "bagian": "Bagian Inspect"}, actor_admin)
        ksc = await ins.start(ksc["id"], actor_petugas)
        lines = ksc.get("lines") or []
        # Baris pertama: cacat 24 poin (6 cacat × 4 poin) → grade turun dari A.
        # Warna DIBEDAKAN dari sample supaya kebijakan "tahan" benar-benar terjadi.
        ksc = await ins.inspect_line(
            ksc["id"], lines[0]["id"],
            {"defects": [{"point_value": 4, "count": 6}],
             "gsm_actual": 148, "width_actual": 112,
             "color_result": "beda_shade", "handfeel_result": "sesuai",
             "handfeel_score": 4,
             "remark": "Shade lebih tua dari sample yang di-ACC pada 2 meter awal."},
            actor_petugas)
        # Baris kedua (bila ada): bersih — supaya layar menunjukkan perbedaan antara
        # roll yang lolos dan roll yang ditahan, bukan semuanya merah.
        if len(lines) > 1:
            ksc = await ins.inspect_line(
                ksc["id"], lines[1]["id"],
                {"defects": [{"point_value": 1, "count": 3}],
                 "gsm_actual": 152, "width_actual": 114,
                 "color_result": "sesuai", "handfeel_result": "sesuai",
                 "handfeel_score": 5, "remark": "Sesuai sample."},
                actor_petugas)

    # ── (4) retur pelanggan: perjalanan lengkap + SPK yang DIPUTUSKAN ───────────
    # SEMUA retur yang sudah ada saat langkah ini dijalankan diberi SEBAB keluhan
    # (master `complaint_reasons`) supaya laporan "keluhan tersering" punya angka,
    # dan satu retur KSC dijalankan sampai keputusan sehingga garis waktunya terisi
    # keempat tonggak. Retur yang lahir SESUDAH langkah ini (rantai E-9 di akhir
    # seed) sengaja dibiarkan tanpa sebab keluhan — itu contoh JUJUR dokumen lama
    # yang harus tampil "—" di layar, bukan "0" atau kode palsu.
    returns = await db.sales_returns.find({}, {"_id": 0}).sort("number", 1).to_list(20)
    kode_urut = ["warna_beda", "kotor_stain", "kurang_yard", "sobek_lubang"]
    catatan_urut = [
        "Pelanggan melaporkan warna lebih tua dari sample yang di-ACC.",
        "Noda oli pada 3 meter bagian tengah roll.",
        "Panjang aktual kurang 4 yard dari surat jalan.",
        "Sobek memanjang di tepi kain.",
    ]
    spk_retur_dibuat = False
    for i, r in enumerate(returns):
        kode = kode_urut[i % len(kode_urut)]
        catatan = catatan_urut[i % len(catatan_urut)]
        try:
            await ret_svc.set_complaint(r["id"], kode, catatan, admin["name"])
            await ret_svc.mark_milestone(r["id"], "shipped_to_store", admin["name"],
                                         at=ago(days=6), note="SJ Kirim Toko diterbitkan")
            await ret_svc.mark_milestone(r["id"], "goods_arrived", admin["name"],
                                         at=ago(days=4), note="Barang sampai di gudang")
        except Exception as exc:  # noqa: BLE001
            print(f"  [warn] perjalanan retur {r.get('number')} dilewati: {exc}")
            continue
        if r.get("entity_id") == "ent_ksc" and not spk_retur_dibuat:
            spk_retur_dibuat = True
            # SPK inspeksi retur: dibuat, dikerjakan, lalu DIPUTUSKAN "terima sebagian"
            # → `inspect_done_at` retur terisi dari dokumennya (satu tanggal, satu sumber).
            try:
                doc = await ins.create(
                    {"kind": "return_customer", "ref_doc_id": r["id"],
                     "assigned_to": (petugas or {}).get("id", ""),
                     "remark": "Periksa keluhan warna terhadap sample yang di-ACC."},
                    actor_admin, "ent_ksc")
                made += 1
                doc = await ins.start(doc["id"], actor_admin)
                for ln in (doc.get("lines") or [])[:1]:
                    doc = await ins.inspect_line(
                        doc["id"], ln["id"],
                        {"color_result": "beda_shade", "handfeel_result": "sesuai",
                         "decision": "terima_sebagian",
                         "remark": "Keluhan pelanggan TERBUKTI: shade beda dari sample."},
                        actor_admin)
                await ins.finish(doc["id"], "terima_sebagian",
                                 "Keluhan warna terbukti — 1 roll diterima dengan "
                                 "potongan, sisanya dikembalikan ke supplier.",
                                 actor_admin)
            except Exception as exc:  # noqa: BLE001
                print(f"  [warn] SPK inspeksi retur {r.get('number')} dilewati: {exc}")

    tahan = await db.inspections.count_documents({"lines.hold": True})
    total = await db.inspections.count_documents({})
    print(f"✅ SPK Inspeksi seeded ({total} dokumen · {tahan} ber-baris DITAHAN "
          f"menunggu keputusan manajer) — FASE I")
    return total
...
async def seed_design_requests():
    ...
    mgr = {"id": "user_admin_01", "name": "Budi Santoso", "role": "admin"}
    designer = {"id": "user_designer_01", "name": "Sari Melati", "role": "designer"}
    from services import design_gallery_service as _gal  # noqa: PLC0415
    from services import design_studio_service as _studio  # noqa: PLC0415

    so = await db.sales_orders.find_one({"entity_id": ENT}, {"_id": 0, "id": 1})
    made = 0

    async def buat(brief: str, cats: tuple, due_days: int, source: str = "internal",
                   so_id: str = "", line_code: str = "") -> dict:
        return await drs.create({
            "source": source, "so_id": so_id, "line_code": line_code,
            "category_code": cats[0], "design_category_code": cats[1], "brief": brief,
            "due_date": (datetime.now(timezone.utc) + timedelta(days=due_days)).date().isoformat(),
            "submit_now": True,
        }, md, ENT)

    async def kerjakan(req: dict, judul: str) -> str:
        """Alur SINKRON: desain Studio dibuat dari permintaan + satu berkas artwork → diajukan."""
        res = await drs.create_design_from_request(req["id"], designer, ENT, judul)
        gid = res["design"]["id"]
        await _gal.add_file(gid, "artwork.png", "image/png", _PNG_1PX, kind="artwork",
                            uploaded_by=designer["name"])
        await _studio.transition(gid, "submit", designer, entity_id=ENT)
        return gid

    # (1) menunggu penugasan
    await buat("Motif batik pesisir untuk katalog lebaran — 3 alternatif warna.", ("BTK", "AO"), 7)
    made += 1

    # (2) sedang dikerjakan — desain Studio sudah dibuat dari permintaan (masih draf)
    d2 = await buat("Pattern salur kemeja pria dari kain endek — ukuran S sampai XL.", ("SLR", "AO"), 5)
    await drs.assign(d2["id"], md, designer["id"])
    await drs.create_design_from_request(d2["id"], designer, ENT, "Salur Endek Kemeja")
    made += 1

    # (3) menunggu keputusan — desainer mengajukan di Studio, permintaan mengikuti
    d3 = await buat("Artwork printing motif parang pinggiran untuk pesanan pelanggan.", ("BTK", "PG"), 3,
                    source="so" if so else "internal", so_id=so["id"] if so else "", line_code="printing")
    await drs.assign(d3["id"], md, designer["id"])
    await kerjakan(d3, "Parang Pinggiran Pesanan")
    made += 1

    # (4) sudah ACC — lewat satu putaran revisi ber-alasan & nilai, semuanya di Studio
    d4 = await buat("Motif kawung modern (bunga) untuk kain seragam kantor.", ("BGA", "AO"), 10)
    await drs.assign(d4["id"], md, designer["id"])
    gid4 = await kerjakan(d4, "Kawung Modern Seragam")
    await _studio.transition(gid4, "request_revision", mgr, note="Skala motif terlalu besar untuk kain 115 cm.",
                             score=1.0, entity_id=ENT)
    await _gal.add_file(gid4, "artwork_v2.png", "image/png", _PNG_1PX, kind="artwork", uploaded_by=designer["name"])
    await _studio.transition(gid4, "submit", designer, entity_id=ENT)
    await _studio.transition(gid4, "approve", mgr, note="Sudah sesuai contoh pelanggan.", score=1.75, entity_id=ENT)
    made += 1
...
async def seed_interco():
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://asgi",
        ...
        rs = await cm.post("/api/interco/settlements", headers=H(mgr, BUYER), json={
            ...
            # F-08 (audit 2026-09) — netting hanya sah bila ada piutang balik Kanda→KSC;
            # data demo melunasi lewat transfer antar rekening PT.
            "method": "transfer",
            "notes": "Pelunasan saldo antar-PT lewat transfer (data demo G-6)"})
    ...
    print(f"✅ Transaksi antar-PT (Fase G-6): {made['tx']} transaksi "
          ...
          f"{trf['code']} tanpa jurnal dobel · 1 lunas lewat transfer antar rekening · 1 dikonfirmasi "
...
def _artwork_parang_png() -> bytes:
    """Artwork DSG-PARANG-01 (monokrom, 480×360) — dibuat, bukan 1×1 px (iter281)."""
    from io import BytesIO  # noqa: PLC0415
    from PIL import Image, ImageDraw  # noqa: PLC0415
    w, h = 480, 360
    img = Image.new("RGB", (w, h), (245, 245, 243))
    d = ImageDraw.Draw(img)
    for i in range(-h, w, 44):
        d.line([(i, h), (i + h, 0)], fill=(26, 26, 26), width=16)
        d.line([(i + 24, h), (i + 24 + h, 0)], fill=(120, 120, 120), width=4)
    for gy in range(22, h, 66):
        for gx in range(22, w, 66):
            d.ellipse([gx - 4, gy - 4, gx + 4, gy + 4], fill=(26, 26, 26))
    buf = BytesIO()
...
async def seed_rnd():
    ...
    design_id = ""
    try:
        # Kode desain kini dibangkitkan Design Studio → cari berdasarkan JUDUL.
        existing = await db.design_gallery.find_one(
            {"$or": [{"code": "DSG-PARANG-01"}, {"title": "Parang Modern Monokrom"}]},
            {"_id": 0, "id": 1, "status": 1, "files": 1})
        if existing:
            design_id = existing["id"]
            # iter281: ganti artwork 1×1 px lama dengan gambar yang terlihat.
            for _f in (existing.get("files") or []):
                if _f.get("filename") == "artwork_parang.png" and int(_f.get("size") or 0) < 200:
                    await _dg.delete_file(design_id, _f["id"])
                    await _dg.add_file(design_id, "artwork_parang.png", "image/png",
                                       _artwork_parang_png())
            ...
            design_id = dsg["id"]
            await _dg.add_file(design_id, "artwork_parang.png", "image/png", _artwork_parang_png())
            # F-6.7 — draf WAJIB diajukan dulu sebelum disahkan.
            await _dg.submit_design(design_id, RND)
    ...
    # ── 4. Permintaan sampling DUA JENIS (labdip + handfeel) ke 2 supplier ───
    # FASE S user story S.F-1: satu permintaan menempuh dua jenis sekaligus, dan
    # tiap jenis punya rangkaian round sendiri. Data demo dibuat begini supaya
    # layar yang membanding-bandingkan supplier PER JENIS tidak pernah kosong.
    ...
    # FASE S user story S.F-2: permintaan ditautkan ke PESANAN, sehingga dari layar
    # pesanan itu sample-nya bisa ditemukan (lewat Jejak Dokumen).
    so = await db.sales_orders.find_one({"entity_id": ent, "order_number": "SO-0007"},
                                        {"_id": 0, "id": 1, "order_number": 1,
                                         "customer_id": 1}) \
        or await db.sales_orders.find_one({"entity_id": ent},
                                          {"_id": 0, "id": 1, "order_number": 1,
                                           "customer_id": 1})

    smp1 = await _smp.create_sample({
        "spec_id": spec_a["id"], "sample_types": ["labdip", "handfeel"],
        "title": "Sampling Katun Combed 150 gsm — warna khusus (labdip + handfeel)",
        "brief": "Cocokkan warna target maksimal ΔE 1.5 DAN nilai rasa kainnya. "
                 "Kirim swatch 3 meter + hasil ukur.",
        "color_target": {"color_id": color_id},
        "so_id": (so or {}).get("id", ""),
    ...
    async def _do_round(sample_id, supplier_id, type_code, note, meas, cost, result,
                        score, assess_note):
        """Satu siklus round penuh: unggah bukti → setor hasil → dinilai.

        FASE S — `type_code` WAJIB: dengan dua jenis berjalan, satu supplier punya
        DUA round terbuka sekaligus, dan memilih "yang terbuka" tanpa menyebut jenis
        akan menyetor hasil handfeel ke rangkaian labdip.
        """
        ...
        await _smp.add_attachment(sample_id, rnd_row["id"], "hasil_sample.png",
    ...
    # Hasil ukur SENGAJA berbeda per jenis — daftarnya lahir dari master
    # (`sample_types.measurement_fields`), jadi mengirim `delta_e` pada handfeel
    # akan DITOLAK server. Itu perilaku yang diinginkan, bukan kekakuan.
    await _do_round(smp1["id"], s1["id"], "labdip",
                    "Warna sangat dekat dengan target.",
                    {"delta_e": 0.9, "colorfastness_wash": 4, "colorfastness_rub": 4},
                    165000, "acc", 92, "ΔE 0.9 — paling presisi di antara peserta.")
    await _do_round(smp1["id"], s1["id"], "handfeel",
                    "Rasa kain lembut & jatuh, gramasi tepat.",
                    {"gsm_actual": 151, "lebar": 160, "shrinkage_pct": 2,
                     "handfeel_score": 5},
                    120000, "acc", 90, "Handfeel 5 — sesuai contoh pelanggan.")
    await _do_round(smp1["id"], s2["id"], "labdip",
                    "Warna satu tingkat lebih tua dari target.",
                    {"delta_e": 2.4, "colorfastness_wash": 3, "colorfastness_rub": 4},
                    140000, "revisi", 68, "Minta perbaikan: turunkan intensitas warna.")
    await _do_round(smp1["id"], s2["id"], "handfeel",
                    "Kain agak kaku dibanding contoh.",
                    {"gsm_actual": 148, "lebar": 158, "shrinkage_pct": 3,
                     "handfeel_score": 3},
                    110000, "revisi", 64, "Minta pelembut ditambah — rasa masih kaku.")
    # Round 2 hanya untuk LABDIP: kuota iterasi dihitung per (supplier × jenis),
    # jadi perbaikan warna tidak memakan kuota perbaikan handfeel.
    await _smp.open_round(smp1["id"], s2["id"], type_code="labdip",
                          note="Perbaikan intensitas warna", actor=ADMIN)
    await _do_round(smp1["id"], s2["id"], "labdip",
                    "Sudah lebih muda, mendekati target.",
                    {"delta_e": 1.4, "colorfastness_wash": 4, "colorfastness_rub": 4},
    ...
    # ── 6b. FASE S (user story S.F-4) — sample JADI lalu DIKIRIM ─────────────
    # Dua peristiwa berbeda dengan tanggal masing-masing; tujuan pengiriman WAJIB.
    # Tanpa data demo ini, kolom "Sample jadi / Dikirim" di layar selalu "Belum" dan
    # fitur yang baru dibangun tidak terlihat sama sekali saat demo.
    cust = await db.customers.find_one({"id": (so or {}).get("customer_id", "")},
                                       {"_id": 0, "name": 1}) or {}
    await _smp.finish_sample(smp1["id"],
                             {"note": "Swatch pemenang sudah dijilid & diberi label."},
                             RND)
    await _smp.deliver_sample(smp1["id"], {
        "to": "customer", "to_name": cust.get("name", "") or "Pelanggan korporat",
        "note": "Dikirim bersama surat pengantar untuk persetujuan akhir."}, RND)
    print(f"   · {smp1['number']} ditandai JADI lalu DIKIRIM ke pelanggan "
          f"{cust.get('name', '—')} (FASE S)")

    # ── 7. Permintaan proofing yang MASIH BERJALAN (papan SLA tidak kosong) ──
    smp2 = await _smp.create_sample({
        "spec_id": spec_b["id"], "sample_types": ["proofing"],
...
async def _settle_turn_notifications() -> None:
    """Jalankan sapuan "Giliran Anda" SEKARANG, bukan menunggu penjadwal (tiap 5 menit).

    Terukur 2026-09-22 (INV-GATE-01): seed menghapus `notifications` tetapi tidak
    `turn_marks`, lalu `turn_scan` penjadwal menyala DI TENGAH gate → +34 notifikasi
    "Giliran Anda" muncul sebagai residu gate padahal itu jejak seed yang tertunda.
    Tanda lama dibuang (dokumennya sudah lahir ulang dengan id baru) dan sapuan
    dijalankan di sini supaya keadaan pasca-seed TENANG & deterministik.
    """
    sys.path.insert(0, str(Path(__file__).resolve().parent / "backend"))
    from db import db as _bdb                                   # noqa: PLC0415
    from services import turn_notification_service as _turn     # noqa: PLC0415
    await _bdb.turn_marks.delete_many({})
    res = await _turn.scan()
    print(f"✅ Pemberitahuan giliran disapu saat seed: {res['created']} notifikasi dari {res['scanned']} dokumen")
...
async def main():
    ...
    print(f"  - Induk produk: {_pv['products_linked']} varian ditautkan, yatim {_pv['orphans_left']}")
    await _settle_turn_notifications()
