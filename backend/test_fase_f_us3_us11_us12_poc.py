...
from poc_stock_guard import (STOCK_COLLECTIONS, restore_stock,  # noqa: E402
                            snapshot_stock)
...
def main():
    ...
    # `credit_overrides` ikut di-snapshot (POC-RESIDU-04): US3 menguji pagar
    # kredit dan meninggalkan 1 pembebasan (`credit_overrides` 0 → 1). Pembebasan
    # yang tertinggal membuat POC/uji BERIKUTNYA lolos pagar kredit tanpa ada yang
    # tahu — kelas "hijau karena keadaan warisan", bukan karena kodenya benar.
    stock_snap = snapshot_stock(STOCK_COLLECTIONS + ["credit_overrides"])
    before = _db_counts()
    us3(sales, admin)
    us11(warehouse)
    us12(admin)
