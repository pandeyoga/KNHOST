...
SCOPED_COLLECTIONS = {
    ...
    "wms_tasks", "shipments",
    "goods_receipts", "supplier_dn_profiles",   # GRN Fase 2
    ...
    "logistics_deliveries",   # FB-02 — pengiriman logistik (foto muat/POD, posisi)
    "fleet_vehicles",         # Armada internal (master kendaraan per badan usaha)
    "tax_pph_records",
...
async def guard_doc(request: Request, collection: str, doc: Optional[Dict[str, Any]],
                    *, strict: bool = True, not_found: str = "Data tidak ditemukan") -> EntityContext:
    """Audit 2026-09-21 (KN-A01..A07, B29..B35) — penjaga entitas SATU BARIS untuk jalur TULIS.

    Dokumen wajib milik badan usaha aktif (`strict=True`, sama ketatnya dengan daftar)
    atau minimal berada di penugasan akun (`strict=False`). 404 bila dokumen tak ada.
    """
    if not doc:
        raise HTTPException(status_code=404, detail=not_found)
    ctx = await entity_ctx(request)
    if strict:
        assert_active_entity_access(doc, collection, ctx)
    else:
        assert_entity_access(doc, collection, ctx)
    return ctx
