...
from poc_stock_guard import (CASH_COLLECTIONS, LEDGER_COLLECTIONS,  # noqa: E402
                            restore_stock, snapshot_stock, sweep_ghost_refs)
...
def cleanup(admin: str, ctx: Dict[str, Any]) -> None:
    ...
    # INV-REF-04 — kasus keuangan & kwitansi POC bertaut DUA ARAH ke pesanan demo
    # (`settles`/`settled_by`). Menghapus kasusnya saja meninggalkan `SO-0008` &
    # `KSC/SO-00010` memegang 7 tautan ke dokumen yang sudah tidak ada — terukur
    # pada `gate.sh --full`, dan itu menjatuhkan `audit_doc_refs --strict`.
    swept = sweep_ghost_refs()
    ok(sweep_ghost_refs(verbose=False) == 0,
       f"nol tautan hantu sesudah kasus & kwitansi POC dihapus (INV-REF-04 · {swept} disapu)")
...
def main() -> int:
    ...
    # POC-RESIDU-03 — `cleanup()` di bawah sengaja BEDAH (agar kasus demo tak ikut
    # terhapus), tetapi bedah tidak pernah bisa seteliti snapshot: terukur
    # 2026-08-21 POC ini masih meninggalkan `journal_entries +3` dan
    # `cash_transactions +1`. Jurnal & kas bukan dokumen bernomor yang perlu
    # dijaga selektif, jadi keduanya dipulihkan EKSAK di akhir.
    _money_snap = snapshot_stock(LEDGER_COLLECTIONS + CASH_COLLECTIONS)
    try:
