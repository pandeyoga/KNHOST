...
# 1×1 px PNG — berkas artwork contoh untuk siklus hidup Design Studio (ronde 1 & 2).
...
def main() -> int:  # noqa: C901 — POC linear supaya terbaca sebagai bukti
    ...
    notif_before_ids = {d["id"] for d in db.notifications.find({}, {"_id": 0, "id": 1})}
    ...
    # Sinkron 2026-09-18: desain Galeri menjalani siklus hidup DESIGN STUDIO (punya
    # `versions[]`) → keputusan (revisi/ACC) diambil di halaman desainnya dan status
    # permintaan MENGIKUTI otomatis. Pintu lama di permintaan MENOLAK (400) — bukti
    # bahwa hanya ada satu pintu keputusan.
    r = mgr.post(f"{BASE}/api/design-requests/{dsr_id}/reject", headers=h(ENT_A),
                 timeout=30, json={"reason": "coba lewat pintu lama"})
    ok(r.status_code == 400 and "design studio" in r.text.lower(),
       "pintu lama `/reject` di permintaan ditolak 400 — keputusan hanya lewat Design Studio",
       f"HTTP {r.status_code} · {r.text[:100]}")

    # desainer mengunggah berkas artwork ronde 1 lalu MENGAJUKAN (pending_approval)
    r = dsg.post(f"{BASE}/api/design-gallery/{art['id']}/files-kind/artwork",
                 timeout=30, files={"file": (f"poc-{tag}-r1.png", _PNG_1PX, "image/png")},
                 headers={**h(ENT_A), "Content-Type": None})
    ok(r.status_code == 200, "desainer mengunggah berkas artwork ronde 1",
       f"HTTP {r.status_code} · {r.text[:100]}")
    r = dsg.post(f"{BASE}/api/design-gallery/{art['id']}/lifecycle/submit", headers=h(ENT_A),
                 timeout=30, json={"note": "ronde 1"})
    ok(r.status_code == 200 and r.json().get("status") == "pending_approval",
       "desainer mengajukan desain → `pending_approval`", f"HTTP {r.status_code} · {r.text[:100]}")

    r = mgr.post(f"{BASE}/api/design-gallery/{art['id']}/lifecycle/request-revision",
                 headers=h(ENT_A), timeout=30, json={"note": ""})
    ok(r.status_code == 400 and "wajib" in r.text.lower(),
    ...
    r = mgr.post(f"{BASE}/api/design-gallery/{art['id']}/lifecycle/request-revision",
                 headers=h(ENT_A), timeout=30, json={"note": alasan})
    ok(r.status_code == 200 and r.json().get("status") == "revision"
       and int(r.json().get("version") or 0) == 2,
       "minta revisi di Studio → desain `revision` + ronde 2 dibuka otomatis",
       f"HTTP {r.status_code} · versi={r.json().get('version')}")
    r = mgr.get(f"{BASE}/api/design-requests/{dsr_id}", headers=h(ENT_A), timeout=30)
    ok(r.status_code == 200 and r.json()["status"] == "revision"
       and r.json().get("reject_reason") == alasan and r.json().get("revision_count") == 1,
       "permintaan MENGIKUTI: status `revision` + alasan + `revision_count` naik",
    ...
    # ── revisi diserahkan lagi (ronde 2) → ACC ──────────────────────────────
    r = dsg.post(f"{BASE}/api/design-gallery/{art['id']}/files-kind/artwork",
                 timeout=30, files={"file": (f"poc-{tag}-r2.png", _PNG_1PX, "image/png")},
                 headers={**h(ENT_A), "Content-Type": None})
    ok(r.status_code == 200, "desainer mengunggah berkas artwork ronde 2", f"HTTP {r.status_code}")
    r = dsg.post(f"{BASE}/api/design-gallery/{art['id']}/lifecycle/submit", headers=h(ENT_A),
                 timeout=30, json={"note": "versi 2 (revisi)"})
    rq = mgr.get(f"{BASE}/api/design-requests/{dsr_id}", headers=h(ENT_A), timeout=30)
    ok(r.status_code == 200 and rq.json().get("status") == "delivered",
       "desainer menyerahkan hasil revisi → permintaan kembali ke `delivered`",
       f"HTTP {r.status_code} · {rq.json().get('status')}")

    r = mgr.post(f"{BASE}/api/design-gallery/{art['id']}/lifecycle/approve", headers=h(ENT_A),
                 timeout=30, json={"note": "ACC POC", "score": 2})
    rq = mgr.get(f"{BASE}/api/design-requests/{dsr_id}", headers=h(ENT_A), timeout=30)
    ok(r.status_code == 200 and rq.json().get("status") == "approved"
       and rq.json().get("decided_by"),
       "manajer ACC di Studio → permintaan `approved` + pemutus tercatat",
       f"HTTP {r.status_code} · {r.text[:80] if r.status_code != 200 else rq.json().get('decided_by')}")
    ...
    # Notifikasi siklus hidup Studio (diajukan/revisi/ACC) lahir dari desain POC — dibuang
    # dengan sidik jari id yang sama.
    baru_n = {d["id"] for d in db.notifications.find({}, {"_id": 0, "id": 1})} - notif_before_ids
    if baru_n:
        db.notifications.delete_many({"id": {"$in": list(baru_n)}})
