...
from poc_stock_guard import (CONFIG_COLLECTIONS, STOCK_COLLECTIONS,  # noqa: E402
                            restore_stock, snapshot_stock)
...
def test_registry() -> Dict[str, Any]:
    ...
    for doc in coll.find({}, {"_id": 0}):
        scope = doc.get("scope")
        # `__migrations__` = penanda migrasi sekali-jalan (bootstrap), bukan setting pemilik.
        if scope in (None, "alerts", "__migrations__"):
            continue
...
def cleanup() -> None:
    ...
    for item in CLEANUP:
        ...
        if "product_id" in item:
            prod = db.products.find_one({"id": item["product_id"]}, {"_id": 0, "template_id": 1}) or {}
            db.products.delete_one({"id": item["product_id"]})
            tid = prod.get("template_id")
            if tid and db.products.count_documents({"template_id": tid}) == 0:
                db.product_templates.delete_one({"id": tid})   # induk yang lahir dari produk uji
...
def main() -> int:
    ...
    try:
        # POC-RESIDU-04 — POC ini MENGUBAH konfigurasi (itu memang temanya), dan
        # `cleanup()` di bawah memulihkan NILAI-nya satu per satu. Yang tidak
        # dipulihkannya: DOKUMEN scope baru yang lahir saat setting ditulis per
        # badan usaha (terukur `system_settings` 3 → 5). Konfigurasi adalah SSOT
        # perilaku aplikasi, jadi dipulihkan EKSAK — bukan diperbaiki per-nilai.
        _stock_snap = snapshot_stock(STOCK_COLLECTIONS + CONFIG_COLLECTIONS)
        test_registry()
...
if __name__ == "__main__":
    raise SystemExit(__import__('poc_stock_guard').with_journal_cleanup(main))
