...
from routers import (
    ...
    purchase_orders_extra, purchase_orders, inbound_receiving_extra, inbound_receiving, inbound_scan_label, goods_receipts, outbound_picking_extra, outbound_picking,
    ...
    enums, uom_conversions, marketing,
...
async def lifespan(app: FastAPI):
    ...
    # PERAN KUSTOM — muat ke registry sebelum request pertama.
    try:
        from services.custom_role_service import sync_registry
        await sync_registry()
...
_cors_raw = (os.environ.get("KN_ALLOWED_ORIGINS") or os.environ.get("CORS_ORIGINS", "")).strip()
...
for module in [
    ...
    marketing,        # Marketing — sosial media & kalender konten
...
from routers import wilayah  # noqa: E402  — wilayah & kode pos tervalidasi (2026-09)
app.include_router(wilayah.router)
from routers import data_hygiene  # noqa: E402  — Kebersihan Data (EYD/telepon otomatis) + peta giliran (2026-09)
app.include_router(data_hygiene.router)
from routers import sample_sales  # noqa: E402  — §3-C Jual Sampel (2026-09)
app.include_router(sample_sales.router)
from routers import sample_orders  # noqa: E402  — REVISI SAMPEL: Pesanan Sampel (SMP-) + Meja Admin Sampel
