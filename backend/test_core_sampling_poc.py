...
def main() -> int:
    ...
    # `md_specs` ikut: sejak iter 43 setiap permintaan sample lahir bersama spesifikasinya.
    TRAIL_COLLS = ("audit_logs", "notifications", "sessions", "approval_matrix_log", "md_specs")
    ...
    body = {"sample_types": ["proofing", "handfeel"],
            ...
            # iter 43 — spesifikasi produk WAJIB lahir bersama permintaan sample.
    ...
    r = admin.post(f"{BASE}/api/rnd/samples/{smp['id']}/rounds/{row_pf['id']}/submit",
                   headers=h(), timeout=60,
                   json={"note": "ada catatan tapi belum ada bukti", "measurements": {}})
    ...
    # MD-05: proofing = foto + catatan saja; hasil ukur DITOLAK bila dikirim.
    r = admin.post(f"{BASE}/api/rnd/samples/{smp['id']}/rounds/{row_pf['id']}/submit",
                   headers=h(), timeout=60,
                   json={"note": "coba kirim hasil ukur",
                         "measurements": {"delta_e": 3.4}, "cost": 120000})
    ok(r.status_code == 400 and "tidak diminta" in r.text,
       "round PROOFING menolak hasil ukur (MD-05: cukup foto + catatan)", f"{r.status_code} {r.text[:200]}")
    r = admin.post(f"{BASE}/api/rnd/samples/{smp['id']}/rounds/{row_pf['id']}/submit",
                   headers=h(), timeout=60,
                   json={"note": "Cetak agak buram, register bergeser.",
                         "measurements": {}, "cost": 120000})
    ok(r.status_code == 200, "hasil proofing s1 disetor (foto + catatan)", r.text[:150])
    doc = get_sample(admin, smp["id"])
    ...
    ok(len(extra) <= 1,
       ...
       f"baris baru={extra}")
    # Pelaksana round POC (akun admin) boleh sudah punya baris di rapor data demo —
    # angkanya memang bergerak karena dia yang mengerjakan round uji; bukan regresi.
    pelaksana = (admin.get(f"{BASE}/api/auth/me", headers=h(), timeout=30).json() or {}).get("name")
    geser = [k for k in base_rows
             if k != pelaksana
             and (base_rows[k].get("grade"), base_rows[k].get("rounds"),
