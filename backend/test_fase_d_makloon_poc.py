...
from poc_stock_guard import restore_stock, snapshot_stock, sweep_ghost_refs  # noqa: E402
...
async def cleanup(db):
    ...
    # KELIRU SELAMA INI: field-nya `notif_type`, BUKAN `type` — jadi filter lama
    # tidak pernah cocok dan POC ini sebenarnya TIDAK PERNAH membersihkan
    # notifikasinya. Sekadar membetulkan nama field justru BERBAHAYA: ia akan
    # menghapus notifikasi makloon milik DATA DEMO juga. Karena itu sapuannya
    # dibatasi ke SPK milik POC lewat `action_id`/`ref` (lihat
    # `services/makloon_claim_service.notify_claim_opened`).
    if oids:
        await db.notifications.delete_many({
            "notif_type": {"$in": ["makloon_claim", "makloon_claim_approval"]},
            "$or": [{"action_id": {"$in": oids}}]
                   + [{"ref": {"$regex": f"(^|:){o}(:|$)"}} for o in oids]})
    # INV-REF-04 — SPK & tagihan uji bertaut DUA ARAH ke kontrak/PO. Menghapus
    # dokumennya saja meninggalkan induk yang MASIH HIDUP menunjuk ketiadaan
    # (panel "Referensi Dokumen" menawarkan tautan 404, `audit_doc_refs` MERAH).
    sweep_ghost_refs()
...
async def main():  # noqa: C901 — satu skrip POC lengkap (protokol repo)
    ...
    db = client[DB_NAME]
    # POC-RESIDU-04 — POC ini menulis kebijakan klaim makloon, dan penulisan itu
    # MELAHIRKAN dokumen scope baru di `system_settings` (terukur +1 tiap jalan).
    # `system_settings` adalah SSOT perilaku aplikasi, jadi dipulihkan EKSAK.
...
if __name__ == "__main__":
    sys.exit(0 if __import__('poc_stock_guard').with_journal_cleanup(lambda: asyncio.run(main())) else 1)
