...
class ProductPayload(BaseModel):
    ...
    base_fabric_template_id: str = ""     # kain dasar (induk master data sumber, mis. woven polos utk printing)
    variant_attrs: Dict[str, Any] = {}    # F1b — nilai axis varian (warna/grade/asal)
...
class ProductTemplateCreate(BaseModel):
    ...
    base_fabric_template_id: str = ""     # kain dasar dari master data (opsional)
    line_code: str = ""
    exclusivity: str = "umum"
    owner_sales_ids: List[str] = Field(default_factory=list)
...
class ProductTemplatePatch(BaseModel):
    ...
    base_fabric_template_id: Optional[str] = None
    line_code: Optional[str] = None
    exclusivity: Optional[str] = None
    owner_sales_ids: Optional[List[str]] = None
...
class SalesOrderItemIn(BaseModel):
    ...
    unit_factor_to: str = ""
    # §3-C — "Jual sebagai sampel": harga dari master sampel (atau manual), dipenuhi lewat
    # permintaan potong ke gudang (tugas outbound bersubtipe sample_cut), bukan reservasi FEFO.
    is_sample: bool = False
    sample_price: Optional[float] = Field(None, ge=0)
...
class SalesOrderCreate(BaseModel):
    ...
    # REVISI SAMPEL — jenis pesanan dipilih SEBELUM checkout: "regular" (PO biasa) | "sample".
    # Sampel TIDAK boleh digabung dengan baris roll biasa; nomor & alur persetujuannya terpisah.
    order_type: str = "regular"
    sample_billing: str = ""               # "free" | "paid" — wajib bila order_type=="sample"
