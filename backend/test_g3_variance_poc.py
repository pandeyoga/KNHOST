...
from poc_stock_guard import FULL_COLLECTIONS, restore_stock, snapshot_stock  # noqa: E402
...
def main() -> int:  # noqa: C901 — POC memang panjang & berurutan
    ...
    _stock_snap = snapshot_stock(FULL_COLLECTIONS)
    ...
    # KEB-PDPT — kedua pesanan BELUM dikirim: seluruh kas (pelunasan so8 + kelebihan yang
    # dialihkan ke so3) tetap kewajiban Uang Muka Pelanggan 2-1400; pengalihan antar
    # pesanan yang sama-sama belum berpendapatan tidak memindahkan apa pun ke Piutang.
    ok(abs((gl_uang_muka_b - gl_balance("2-1400")) - round(t8 + alloc_amt, 2)) < 0.02,
       "GL Uang Muka Pelanggan 2-1400 bertambah tepat sebesar seluruh kas (pesanan belum "
       "dikirim → uang muka, bukan piutang; pengalihan tak menyentuh 1-1200)",
       f"Δ2-1400={rp(gl_uang_muka_b - gl_balance('2-1400'))} vs kas {rp(t8 + alloc_amt)}")
