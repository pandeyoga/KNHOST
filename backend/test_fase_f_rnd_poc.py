...
def meas_for(meta: dict, type_code: str, **override) -> dict:
    """Hasil ukur yang MEMENUHI master untuk satu jenis sampling (dibaca, bukan ditebak).

    Sebelum FASE S skema round punya 5 field hasil ukur TETAP, jadi POC ini boleh
    menuliskan tiga field pilihannya sendiri. Sejak FASE S kolom & kewajibannya lahir
    dari master `sample_types.measurement_fields` dan divalidasi server dua arah
    (field yang tidak diminta DITOLAK, field yang diminta WAJIB ada). POC yang tetap
    hardcode akan memerah pada kode yang benar — itu yang terjadi: `gsm_actual` &
    `shrinkage_pct` bukan hasil ukur labdip, dan `colorfastness_*` yang diminta
    master tidak pernah dikirim.

    Nilai bawaan diambil dari batas wajar kamus `/api/rnd/meta.measurements` (seperempat
    rentang — di DALAM batas, bukan di tepinya), lalu ditimpa nilai yang memang sedang
    diuji (mis. ΔE 2,4 untuk skenario "warna masih terlalu tua").
    """
    dic = {m["value"]: m for m in (meta.get("measurements") or [])}
    fields = next((t.get("measurement_fields") or []
                   for t in (meta.get("sample_types") or [])
                   if t.get("value") == type_code), [])
    out = {}
    for f in fields:
        d = dic.get(f) or {}
        lo = float(d.get("min") if d.get("min") is not None else 1)
        hi = float(d.get("max") if d.get("max") is not None else 5)
        out[f] = round(lo + (hi - lo) / 4.0, 2)
    out.update({k: v for k, v in override.items() if k in fields})
    return out
...
def main() -> int:  # noqa: C901 — POC memang panjang & berurutan
    sys.path.insert(0, "/app/backend")
    from poc_stock_guard import restore_stock, snapshot_stock  # noqa: PLC0415
    tok = {k: login(k) for k in USERS}
    admin, manager, sales, wh = tok["admin"], tok["manager"], tok["sales"], tok["warehouse"]
    # POC-RESIDU-04 — persetujuan sample/spesifikasi menulis `approval_matrix_log`
    # (terukur +2 tiap jalan). Baris itu adalah CATATAN KEPUTUSAN: kalau tertinggal,
    # layar matriks persetujuan demo memuat keputusan yang tak pernah diambil siapa
    # pun, dan rapor "berapa kali manajer menyetujui" jadi bohong sedikit-sedikit.
    _appr_snap = snapshot_stock(["approval_matrix_log"])
    ...
    ok({"labdip", "handfeel", "proofing"} <= types,
       "jenis sampling HIDUP dibaca dari MASTER (labdip · handfeel · proofing — FASE S)",
       str(sorted(types)))
    ok("bulk_sample" not in types,
       "`bulk_sample` TIDAK lagi ditawarkan (dinonaktifkan pemilik, keputusan #4) — "
       "enum & label dokumen lama tetap ada", str(sorted(types)))
    ...
    sub_a = requests.post(f"{BASE}/rnd/samples/{sample['id']}/rounds/{r_a['id']}/submit",
                          ...
                          json={"note": "Warna sedikit lebih tua, handfeel bagus",
                                # FASE S — kolom hasil ukur DARI MASTER (labdip: ΔE +
                                # tahan cuci + tahan gosok); ΔE ditimpa 2,4 karena itu
                                # angka yang diuji skenario ini.
                                "measurements": meas_for(meta, "labdip", delta_e=2.4),
                                "cost": 150000})
    ok(sub_a.status_code == 200, "hasil round 1 supplier A tersimpan dengan bukti + hasil ukur",
       f"HTTP {sub_a.status_code} {sub_a.text[:160]}")
    got = next(r for r in sub_a.json()["rounds"] if r["id"] == r_a["id"])
    ok(got["status"] == "submitted" and float(got["measurements"].get("delta_e")) == 2.4
       and got["measurements"].get("colorfastness_wash") is not None,
       "hasil ukur objektif tercatat — kolomnya yang diminta MASTER untuk labdip",
       f"ΔE {got['measurements'].get('delta_e')} · tahan cuci "
       f"{got['measurements'].get('colorfastness_wash')}")
    ...
    ok(r_a2["round_no"] == 2,
       "nomor round berurut per (supplier × jenis) — rnd 1 → rnd 2 pada labdip")
    ...
    requests.post(f"{BASE}/rnd/samples/{sample['id']}/rounds/{r_a2['id']}/submit",
                  ...
                  json={"note": "Warna sudah pas, ΔE 0.9", "cost": 120000,
                        "measurements": meas_for(meta, "labdip", delta_e=0.9)})
    ...
    requests.post(f"{BASE}/rnd/samples/{sample['id']}/rounds/{r_b['id']}/submit",
                  headers=H(admin), timeout=30,
                  json={"note": "Warna terlalu muda",
                        "measurements": meas_for(meta, "labdip", delta_e=4.8)})
    ...
    nod = requests.post(f"{BASE}/rnd/samples", headers=H(admin), timeout=30, json={
        ...
        "color_target": {"color_id": color["id"]},
        # iter 43 — spesifikasi lahir bersama permintaan; tanpa desain tetap harus ditolak.
        "spec": {"target": {"fabric_type": "woven", "gramasi": 200}, "sku_hint": "POC-PRF-NODSG"}})
    ...
    # Kode desain kini DIBANGKITKAN Design Studio (pola `<desainer>-<kategori>-###`),
    # bukan diketik pemanggil — yang diuji: ada kode, versi 1, status draft.
    ok(bool((design.get("code") or "").strip()) and design.get("version") == 1
    ...
    ok(proof.get("design_code") == design.get("code") and proof.get("design_version") == 2,
    ...
    async def purge(db):
        ...
        # Induk (`product_templates`) yang lahir bersama produk uji; spesifikasi yang
        # menunjuk produk uji tapi tidak tercatat di _made; tag desain yang dihitung
        # `remember_tags` saat desain POC dibuat — semuanya terukur tertinggal (+1/+1/+2).
        tpl_ids = {d.get("template_id") async for d in db.products.find(
            {"id": {"$in": pids}}, {"_id": 0, "template_id": 1})} - {None, ""}
        extra_specs = [d["id"] async for d in db.md_specs.find(
            {"$or": [{"product_id": {"$in": pids}}, {"design_id": {"$in": _made["designs"]}},
                     {"title": {"$regex": "^POC "}}]}, {"_id": 0, "id": 1})]
        spec_ids = list({*spec_ids, *extra_specs})
        poc_tags = set()
        async for d in db.design_gallery.find({"id": {"$in": _made["designs"]}}, {"_id": 0, "tags": 1}):
            poc_tags.update(str(t).strip().lower() for t in (d.get("tags") or []))
        for coll, q in (
            ...
            n += res.deleted_count
        for tid in tpl_ids:
            if await db.products.count_documents({"template_id": tid}) == 0:
                n += (await db.product_templates.delete_many({"id": tid})).deleted_count
        for name_lc in poc_tags:
            row = await db.design_tags.find_one({"name_lc": name_lc}, {"_id": 0, "uses": 1})
            if row and int(row.get("uses") or 0) <= 1:
                await db.design_tags.delete_one({"name_lc": name_lc})
            elif row:
                await db.design_tags.update_one({"name_lc": name_lc}, {"$inc": {"uses": -1}})
...
if __name__ == "__main__":
    sys.exit(__import__('poc_stock_guard').with_journal_cleanup(main))
