...
from core_utils import DEFAULT_ENTITY_ID, now_iso, safe_doc
from db import db
...
from services.receiving_roll_service import (  # noqa: E402 — GRN Fase 1: satu sumber helper
    from_meter as _from_meter, receiving_cfg as _receiving_cfg, to_meter as _to_meter)
from services.inbound_complete_service import grade_or_400 as _grade_or_400  # noqa: E402
...
async def scan_label(task_id: str, payload: ScanLabelIn, request: Request) -> Dict[str, Any]:
    ...
    from services.receiving_roll_service import create_label_roll, resolve_label_input
    from services.goods_receipt_service import guard_legacy_receiving
    await guard_legacy_receiving(task)
    li = await resolve_label_input(task, payload.raw, payload.manual)
    manual, decoded = payload.manual, li["decoded"]
    res = await create_label_roll(task=task, decoded=decoded, sitem=li["sitem"], product=li["product"], po=li["po"],
                                  sup=li["sup"], supplier_id=li["supplier_id"],
                                  owner_entity_id=li["owner_entity_id"], actor=actor, manual=manual)
    return {"roll": _roll_view(res["roll"]), "task": res["task"], "decoded": decoded}
...
async def undo_scan(roll_id: str, request: Request) -> Dict[str, Any]:
    ...
    task = await _load_task(roll["grn_task_id"], request)
    if roll.get("grn_id"):
        raise HTTPException(status_code=409, detail="Roll ini milik Kedatangan Barang (GRN) — batalkan dari layar GRN.")
    from services.receiving_roll_service import undo_receiving_roll
    t = await undo_receiving_roll(roll, task)
