...
async def rollback_initial_stock(roll_id: str) -> None:
    """Kompensasi saga stok awal: hapus roll + mutasi yang sempat lahir."""
    await db.inventory_movements.delete_many({"roll_id": roll_id, "movement_type": "initial_stock"})
...
async def add_initial_stock(payload: RollPayload, request: Request) -> Dict[str, Any]:
    ...
    # INV-ATOMIC-01 (kompensasi saga) — roll baru per permintaan (tak ada dokumen bersama);
    # bila mutasi/rebuild gagal, roll yang sudah lahir DIHAPUS lagi agar stok tidak
    # bertambah tanpa jejak mutasi.
    try:
        ...
        await rollback_initial_stock(roll["id"])
        raise
