...
async def list_templates(request: Request, entity_id: Optional[str] = None) -> List[Dict[str, Any]]:
    await require_permission(request, "marketing", "view")
    ctx = await entity_ctx(request)
    return await ext.list_templates(resolve_list_scope("mkt_templates", {}, ctx, entity_id))


@router.post("/marketing/templates")
async def create_template(payload: TemplateBody, request: Request) -> Dict[str, Any]:
    user = await require_permission(request, "marketing", "create")
    ctx = await entity_ctx(request)
    try:
        doc = await ext.create_template(payload.model_dump(), entity_id=ctx.active_entity_id, actor=user)
    except mkt.MarketingError as exc:
        raise _err(exc) from exc
    await audit(user.get("name", ""), "mkt_template_create", "mkt_template", doc["id"], {"name": doc["name"]})
    return doc


@router.post("/marketing/templates/{tid}/use")
async def use_template(tid: str, request: Request) -> Dict[str, Any]:
    await require_permission(request, "marketing", "create")
    ctx = await entity_ctx(request)
    cur = await db.mkt_templates.find_one({"id": tid}, {"_id": 0})
    if not cur:
        raise HTTPException(status_code=404, detail="Template tidak ditemukan")
    assert_entity_access(cur, "mkt_templates", ctx)
    await ext.mark_template_used(tid)
    return {"ok": True}


@router.delete("/marketing/templates/{tid}")
async def delete_template(tid: str, request: Request) -> Dict[str, Any]:
    ...
    if not cur:
        raise HTTPException(status_code=404, detail="Template tidak ditemukan")
    assert_entity_access(cur, "mkt_templates", ctx)
    if user.get("role") not in mkt.APPROVE_ROLES and cur.get("created_by_id") != user.get("id"):
        raise HTTPException(status_code=403, detail="Hanya pembuat template atau manager/admin yang boleh menghapus.")
    await db.mkt_templates.delete_one({"id": tid})
    await audit(user.get("name", ""), "mkt_template_delete", "mkt_template", tid, {"name": cur.get("name")})
    return {"ok": True}
...
async def duplicate_post(pid: str, payload: DuplicateBody, request: Request) -> Dict[str, Any]:
    user = await require_permission(request, "marketing", "create")
    src = await _post_or_404(pid, request)
    ctx = await entity_ctx(request)
    body = ext.duplicate_payload(src, same_entity=src.get("entity_id") == ctx.active_entity_id, publish_at=payload.publish_at)
    try:
        doc = await mkt.create_post(body, entity_id=ctx.active_entity_id, actor=user)
    except mkt.MarketingError as exc:
        raise _err(exc) from exc
    await db.mkt_posts.update_one({"id": doc["id"]}, {"$set": {"duplicated_from": pid, "history.0.note": f"diduplikat dari {src.get('title', '')}"}})
    await audit(user.get("name", ""), "mkt_post_duplicate", "mkt_post", doc["id"], {"from": pid, "title": doc["title"]})
    return await _post_or_404(doc["id"], request)
