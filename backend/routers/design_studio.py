...
from routers.design_gallery import _perm_manage
...
async def _perm_view(request: Request) -> Dict[str, Any]:
    """Baca Studio: `rnd.view` ATAU `hr.view` (cermin design_gallery._perm_view).
    Didefinisikan lokal supaya INV-AUTH-01 bisa menelusuri penegakan auth-nya."""
    return await require_any_permission(request, [("rnd", "view"), ("hr", "view")])
