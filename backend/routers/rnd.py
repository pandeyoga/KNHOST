...
from schemas_rnd import (IssueMaterialBody, ReasonBody, RoundAssessBody, RoundOpenBody,
                         RoundSubmitBody, SampleDecideBody, SampleDeliverBody,
                         SampleFinishBody, SampleInput, SamplePatch,
...
from services import master_registry as mreg         # FASE S — master jenis sampling
...
async def rnd_meta(request: Request, entity_id: str = Query("")) -> Dict[str, Any]:
    ...
    return {
        ...
        "reasons": smp.reasons(),
        # FASE S — jenis sampling dari MASTER (nilai hidup), bukan dari benih enum:
        # jenis yang baru ditambah pemilik harus muncul di form tanpa restart, dan
        # `bulk_sample` yang dinonaktifkan harus HILANG dari pilihan tanpa hilang
        # dari label dokumen lama.
        "sample_types": await mreg.sample_type_options(eid),
        # Kamus bentuk hasil ukur (label · satuan · batas) — dipakai form setor hasil
        # supaya kolomnya bernama manusia, bukan `gsm_actual`.
        "measurements": dr.enum_items("sample_measurement"),
        "deliver_targets": smp.deliver_targets(),
...
# ═══ FASE S — JENIS SAMPLING (master hidup, dipakai pemilih & POC) ═════════
@router.get("/rnd/sample-types")
async def list_sample_types(request: Request, entity_id: str = Query(""),
                            line: str = Query("", description="saring per lini kerja"),
                            ) -> List[Dict[str, Any]]:
    """Jenis sampling EFEKTIF untuk badan usaha aktif (opsional disaring per lini).

    Endpoint terpisah dari `/rnd/meta` DENGAN SENGAJA: pemilih jenis dipakai juga di
    layar Master (Pengaturan) dan di POC, dan menumpangkannya pada meta membuat
    keduanya harus menarik statistik yang tidak dibutuhkan.
    """
    await require_permission(request, "rnd", "view")
    ctx = await entity_ctx(request)
    return await mreg.sample_type_options(entity_id or ctx.active_entity_id, line)
...
async def create_spec(payload: SpecInput, request: Request) -> Dict[str, Any]:
    ...
    ctx = await entity_ctx(request)
    try:
        from services.rnd_catalog_validation import validate_spec_catalog
        await validate_spec_catalog(payload.model_dump(), actor, ctx)
...
async def get_spec(spec_id: str, request: Request) -> Dict[str, Any]:
    ...
    if doc.get("product_id"):
        from services.catalog_access import product_for, public_product
        doc["product"] = public_product(await product_for(actor, doc['product_id']), actor)
    return doc
...
async def patch_spec(spec_id: str, payload: SpecPatch, request: Request) -> Dict[str, Any]:
    actor = await require_permission(request, "rnd", "create")
    ctx = await entity_ctx(request)
    current = await _spec_guard(spec_id, ctx)
    try:
        from services.rnd_catalog_validation import validate_spec_catalog
        await validate_spec_catalog({**current, **payload.model_dump(exclude_unset=True)}, actor, ctx)
...
async def list_samples(request: Request, entity_id: Optional[str] = Query(None),
                       q: str = Query(""), status: str = Query(""),
                       sample_type: str = Query("", description="FASE S — satu JENIS "
                                                                "(keanggotaan array)"),
                       spec_id: str = Query(""),
                       so_id: str = Query("", description="FASE S — sample milik pesanan"),
                       line: str = Query("", description="FASE L — penyaring lini"),
                       limit: int = Query(200, ge=1, le=500)) -> Dict[str, Any]:
    actor = await require_permission(request, "rnd", "view")
    ctx = await entity_ctx(request)
    ...
    rows = await smp.list_samples(scope, q=q, status=status, sample_type=sample_type,
                                  spec_id=spec_id, so_id=so_id, limit=limit)
...
async def create_sample(payload: SampleInput, request: Request) -> Dict[str, Any]:
    ...
    await audit(actor.get("name", ""), "rnd_sample_created", "md_samples", doc["id"],
                {"number": doc["number"], "types": doc.get("sample_types") or [],
                 "so_id": doc.get("so_id", "")})
    return doc
...
async def send_sample(sample_id: str, payload: SampleSendBody, request: Request) -> Dict[str, Any]:
    """Kirim permintaan ke ≥1 supplier × ≥1 JENIS sekaligus (hasilnya dibandingkan)."""
    ...
    await audit(actor.get("name", ""), "rnd_sample_sent", "md_samples", sample_id,
                {"number": doc.get("number"), "suppliers": payload.supplier_ids,
                 "types": payload.type_codes or (doc.get("sample_types") or [])})
    return doc
...
async def open_round(sample_id: str, payload: RoundOpenBody, request: Request) -> Dict[str, Any]:
    """Buka round berikutnya (rnd 2, 3, …) setelah hasil `revisi`, **per jenis**."""
    actor = await require_permission(request, "rnd", "submit")
    ctx = await entity_ctx(request)
    await _sample_guard(sample_id, ctx)
    try:
        doc = await smp.open_round(sample_id, payload.supplier_id,
    ...
    await audit(actor.get("name", ""), "rnd_round_opened", "md_samples", sample_id,
                {"supplier_id": payload.supplier_id, "type_code": payload.type_code},
...
# ═══ FASE S — PELAKSANAAN: sample JADI → DIKIRIM (user story S.F-4) ════════
# Dua endpoint terpisah, bukan satu "selesai", karena keduanya PERISTIWA BERBEDA
# yang tanggalnya dipakai orang berbeda: "jadi" dipakai R&D menutup pekerjaan,
# "dikirim" dipakai sales menjawab pelanggan. Menggabungkannya berarti salah satu
# tanggal harus ditebak.
#
# Izin: `rnd.submit` (bukan `decide`). Ini pencatatan pelaksanaan, BUKAN keputusan —
# keputusan pemilik sesi ini: tidak butuh persetujuan siapa pun. Alasan itu ditulis
# di `verify_approval_queues.DOOR_EXEMPT` supaya gate INV-APPR-01 tetap menuntut
# salah satu dari dua kemungkinan (antrean ATAU pembebasan beralasan).
@router.post("/rnd/samples/{sample_id}/finish")
async def finish_sample(sample_id: str, payload: SampleFinishBody,
                        request: Request) -> Dict[str, Any]:
    """Tandai sample **JADI** — wajib ada round yang ACC lebih dulu."""
    actor = await require_permission(request, "rnd", "submit")
    ctx = await entity_ctx(request)
    await _sample_guard(sample_id, ctx)
    try:
        doc = await smp.finish_sample(sample_id, payload.model_dump(),
                                      actor.get("name", ""))
    except RndError as exc:
        raise _err(exc) from exc
    await audit(actor.get("name", ""), "rnd_sample_finished", "md_samples", sample_id,
                {"number": doc.get("number"), "finished_at": doc.get("finished_at")},
                reason=payload.note or "")
    return doc


@router.post("/rnd/samples/{sample_id}/deliver")
async def deliver_sample(sample_id: str, payload: SampleDeliverBody,
                         request: Request) -> Dict[str, Any]:
    """Catat sample **DIKIRIM** — tujuan WAJIB (pelanggan/sales/supplier/internal)."""
    actor = await require_permission(request, "rnd", "submit")
    ctx = await entity_ctx(request)
    await _sample_guard(sample_id, ctx)
    try:
        doc = await smp.deliver_sample(sample_id, payload.model_dump(),
                                       actor.get("name", ""))
    except RndError as exc:
        raise _err(exc) from exc
    await audit(actor.get("name", ""), "rnd_sample_delivered", "md_samples", sample_id,
                {"number": doc.get("number"), "to": doc.get("delivered_to"),
                 "to_name": doc.get("delivered_to_name"),
