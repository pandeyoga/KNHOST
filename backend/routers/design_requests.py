...
from db import db
from dependencies import require_any_permission, audit, require_permission
...
async def meta(request: Request) -> Dict[str, Any]:
    # Pesanan khusus (sales) memilih kategori pattern/desain dari meta yang sama,
    # jadi izin `order.create` juga cukup — dropdown kategori di form sales tidak
    # boleh mati diam-diam (audit peran 2026-09-22).
    actor = await require_any_permission(request, [("design_request", "view"), ("order", "create")])
