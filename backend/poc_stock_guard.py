...
# ─────────────────────────────────────────────────────────────────────────────
# prune_orphan_turn_marks — tanda "Giliran Anda" yatim  [BARU 2026-09-22]
# ─────────────────────────────────────────────────────────────────────────────
# `turn_notification_service.after_audit` menulis satu baris `turn_marks`
# (koleksi · doc_id · status) tiap dokumen berpindah tahap, supaya orang tidak
# diberi tahu dua kali. POC menghapus dokumennya langsung dari Mongo → tandanya
# tertinggal (+1..+4 per POC, terukur dengan scripts/ukur_residu_poc.py). Kelas
# yang sama dengan `sweep_ghost_refs`: jejak yang menunjuk dokumen yang tak ada.
def prune_orphan_turn_marks(verbose: bool = True) -> int:
    """Buang `turn_marks` yang dokumennya sudah tidak ada. Return jumlah dibuang."""
    try:
        db = _db()
        gone = []
        for m in db.turn_marks.find({}, {"_id": 1, "doc_id": 1, "collection": 1}):
            coll, doc_id = m.get("collection"), m.get("doc_id")
            if not coll or not doc_id or db[coll].count_documents({"id": doc_id}, limit=1) == 0:
                gone.append(m["_id"])
        if gone:
            db.turn_marks.delete_many({"_id": {"$in": gone}})
        if verbose:
            print(f"  {G if not gone else Y}[poc-turn] {len(gone)} tanda giliran yatim disapu.{X}")
        return len(gone)
    except Exception as exc:  # noqa: BLE001
        print(f"{R}  [poc-turn] sapuan tanda giliran GAGAL: {exc}{X}")
        return 0


# ─────────────────────────────────────────────────────────────────────────────
# snapshot_new_ids / purge_new_ids — jejak audit & notifikasi POC  [BARU 2026-09-22]
# ─────────────────────────────────────────────────────────────────────────────
# Koleksi "catatan" (audit_logs, notifications) tumbuh dari SETIAP aksi POC,
# termasuk login. Ambil sidik `_id` SEBELUM login, hapus yang baru di CLEANUP.
JOURNAL_COLLECTIONS = ["audit_logs", "notifications"]


def snapshot_new_ids(collections: Optional[List[str]] = None) -> Dict[str, set]:
    db = _db()
    return {c: set(db[c].distinct("_id")) for c in (collections or JOURNAL_COLLECTIONS)}


def purge_new_ids(snap: Dict[str, set], verbose: bool = True) -> int:
    """Hapus dokumen yang lahir sesudah `snapshot_new_ids`. Return jumlah dibuang."""
    try:
        db = _db()
        total = 0
        for c, before in snap.items():
            new = [i for i in db[c].distinct("_id") if i not in before]
            if new:
                total += db[c].delete_many({"_id": {"$in": new}}).deleted_count
        if verbose:
            print(f"  [poc-journal] {total} jejak audit/notifikasi POC disapu.")
        return total
    except Exception as exc:  # noqa: BLE001
        print(f"{R}  [poc-journal] sapuan jejak GAGAL: {exc}{X}")
        return 0


def with_journal_cleanup(fn: Callable[[], Any]) -> Any:
    """Jalankan `fn` lalu sapu audit_logs/notifications yang lahir selama itu."""
    snap = snapshot_new_ids()
    try:
        return fn()
    finally:
        purge_new_ids(snap, verbose=False)
