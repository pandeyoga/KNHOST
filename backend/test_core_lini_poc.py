...
def main() -> int:  # noqa: C901 — POC linear supaya mudah dibaca sebagai bukti
    ...
    # §D (varian produk): POST /products melahirkan INDUK (`product_templates`) — induk
    # yang hanya dipakai produk uji ikut dihapus, kalau tidak ia tertinggal (+2 terukur).
    poc_prod_q = {"$or": [{"id": {"$in": cleanup_products}}, {"sku": {"$regex": f"^POC-L.-{tag}$"}}]}
    tpl_ids = {d.get("template_id") for d in db.products.find(poc_prod_q, {"_id": 0, "template_id": 1})} - {None, ""}
    removed += db.products.delete_many(poc_prod_q).deleted_count
    for tid in tpl_ids:
        if db.products.count_documents({"template_id": tid}) == 0:
            removed += db.product_templates.delete_many({"id": tid}).deleted_count
    if temp_user_id:
