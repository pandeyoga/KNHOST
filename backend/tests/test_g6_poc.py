...
def test_US6_netting_dua_transaksi(admin_client):
    ...
    # F-08 (audit 2026-09-02) — netting hanya sah bila PEMBAYAR (Kanda) juga punya PIUTANG
    # balik ke penerima (KSC): terbitkan transaksi arah balik Kanda→KSC senilai ≥ netting.
    need = sum(float(d.get("grand_total") or 0) - float(d.get("settled_amount") or 0)
               - float(d.get("returned_amount") or 0) for d in open_docs[:2])
    rb = admin_client.post(
        f"{BASE}/api/interco/transactions",
        json={
            "seller_entity_id": "ent_kanda",
            "buyer_entity_id": "ent_ksc",
            "pricing_mode": "at_cost",
            "items": [{"product_id": "prod_batik_mega",
                       "quantity": 1, "unit_price": round(need + 1000, 2)}],
            "submit_now": True,
        },
    )
