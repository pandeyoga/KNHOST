...
_TOKENS: dict = {}     # email → token (satu login per modul, bukan per uji)
...
def _sess(email="admin@kainnusantara.id", entity="ent_ksc"):
    ...
    s.headers.update({"Authorization": "Bearer " + _TOKENS[email], "X-Entity-Id": entity})
    return s


@pytest.fixture(scope="module", autouse=True)
def _nol_residu():
    """INV-GATE-01 — uji ini hanya MEMBACA, tetapi login-nya menulis `audit_logs` +
    `sessions`. Terukur 2026-09-22: +5 baris audit per gate (gate anti-residu memerah
    di blok runtime). Jejak login uji dihapus di akhir modul; data demo tak disentuh."""
    from dotenv import dotenv_values
    from pymongo import MongoClient
    env = dotenv_values("/app/backend/.env")
    db = MongoClient(env["MONGO_URL"])[env["DB_NAME"]]
    before = {d["id"] for d in db.audit_logs.find({"action": "login"}, {"_id": 0, "id": 1})}
    yield
    new_ids = [d["id"] for d in db.audit_logs.find({"action": "login"}, {"_id": 0, "id": 1})
               if d["id"] not in before]
    if new_ids:
        db.audit_logs.delete_many({"id": {"$in": new_ids}})
    if _TOKENS:
        db.sessions.delete_many({"token": {"$in": list(_TOKENS.values())}})
