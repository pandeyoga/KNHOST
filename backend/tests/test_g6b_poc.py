...
from poc_stock_guard import sweep_ghost_refs  # noqa: E402  (INV-REF-04)
...
def cleanup():
    ...
    journal = snapshot_new_ids(["audit_logs", "notifications", "roll_cost_history"])
    yield
    ...
    # Transaksi yang POC majukan ke `invoiced` dipulihkan PERSIS dari snapshot.
    for snap in STATE.get("tax_pair_snapshot") or []:
        db.interco_transactions.replace_one({"id": snap["id"]}, snap)
    # Cap faktur pajak pada transaksi yang dipakai POC dipulihkan.
    if STATE.get("tax_pair") and not STATE.get("tax_pair_snapshot"):
    ...
    # INV-REF-04 — cap `tax_faktur_*` di atas SUDAH dipulihkan, tetapi `refs[]`
    # belum: faktur pajak & retur yang lahir dari POC menulis tautan DUA ARAH ke
    # transaksi antar-PT milik DATA DEMO. Menghapus dokumen POC-nya saja
    # meninggalkan 4 tautan hantu terukur — `KSC/IC-00002 --child--> tax_invoice`,
    # `KANDA/IC-00002 --child--> input_tax_invoice`, dan dua `IC-00006 --child-->
    # interco_return` — yang menjatuhkan INV-GATE-01 pada gate berikutnya.
    sweep_ghost_refs()
...
def test_a2_siklus_faktur_pajak_internal_penuh(adm):
    ...
    if not cand:
        # Data demo tidak selalu memuat transaksi yang SUDAH difakturkan: majukan satu
        # transaksi ber-PPN yang barangnya sudah diterima lewat pintu nyata `/invoice`.
        # Dokumen pasangannya di-snapshot dan dipulihkan PERSIS saat pembersihan.
        rec = next((t for t in _txs(adm)
                    if t["role"] == "seller" and t["status"] in ("received", "shipped")
                    and t.get("tax_apply") and not t.get("tax_faktur_out_number")), None)
        assert rec, "butuh satu transaksi ber-PPN yang barangnya sudah diterima"
        STATE["tax_pair_snapshot"] = list(
            _db().interco_transactions.find({"pair_id": rec["pair_id"]}, {"_id": 0}))
        r = adm.post(f"{BASE}/api/interco/transactions/{rec['id']}/invoice", json={"note": "POC G-6b"})
        assert r.status_code == 200, r.text
        cand = next(t for t in _txs(adm) if t["id"] == rec["id"])
