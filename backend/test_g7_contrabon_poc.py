...
from poc_stock_guard import STOCK_COLLECTIONS, restore_stock, snapshot_stock  # noqa: E402
...
def setup(admin: str) -> Dict[str, Any]:
    ...
    # KN-D16 — ambang 3-way kini SATU sumber (`purchasing.bill_*_tolerance_percent`) untuk
    # jalur AP maupun kontrabon. Supaya kasus "+3% lolos AP, tertangkap kontrabon" tetap
    # terbukti, ambang harga PT diturunkan ke 1% SESUDAH faktur B posted (dari Pusat
    # Pengaturan, bukan angka di kode); dikembalikan di uji toleransi & pembersihan.
    ok(cfg_set(admin, "purchasing.bill_price_tolerance_percent", 1, "entity", ENT_A),
       "ambang harga 3-way PT diturunkan ke 1% lewat Pusat Pengaturan (satu sumber D16)")
...
def test_schedule(admin: str) -> None:
    ...
    # FASE N: pengingat BERALAMAT per orang (finance & manajer) → satu notifikasi
    # per penerima, jadi ukuran idempotennya = jumlah TIDAK BERTAMBAH saat diulang.
    cnt0 = dbrun(lambda db: db.notifications.count_documents(
    ...
    cnt = dbrun(lambda db: db.notifications.count_documents(
        {"type": "contra_bon_cycle", "ref": {"$regex": f"^cbcycle:{SUP_A}"}}))
    ok(again.status_code == 200 and cnt0 >= 1 and cnt == cnt0,
       "job idempoten — dijalankan dua kali tidak menggandakan pengingat",
       f"{cnt0} → {cnt} notifikasi (per penerima)")
...
def test_match(admin: str, mgr: str, cb: Dict[str, Any], ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ok("price_variance" in kinds,
       "selisih HARGA terdeteksi di kontrabon (ambang PT 1%) walau lolos jalur AP (5%)",
...
def test_tolerance_is_config(admin: str, mgr: str) -> None:
    ...
    # D16: satu ambang untuk AP & kontrabon. Faktur +3% dibuat pada ambang bawaan (5%)
    # supaya posted, lalu ambang PT diturunkan ke 1% → kontrabon menandai selisihnya.
    cfg_reset(admin, "purchasing.bill_price_tolerance_percent", "entity", ENT_A)
    bill = make_bill(admin, "po_002",
                     [{"product_id": "prod_tenun_ikat", "billed_qty": 100, "price": 206000}],
                     "received", f"{POC_TAG}/INV-C")
    ok(cfg_set(admin, "purchasing.bill_price_tolerance_percent", 1, "entity", ENT_A),
       "ambang harga PT diturunkan ke 1% sebelum kontrabon dibentuk")
    r = api("POST", "/contra-bons", mgr, ENT_A, json={
        "supplier_id": SUP_C, "entity_id": ENT_A, "bills": [{"bill_id": bill["id"]}],
        "notes": f"{POC_TAG} uji toleransi"})
    cb = r.json()
    if "id" not in cb:
        print(f"  !! kontrabon uji toleransi gagal: {r.status_code} {r.text[:300]}")
    made["cbs"].append(cb["id"])
    ok(cb["match_summary"]["exceptions_count"] == 1,
       "harga +3% terdeteksi sebagai selisih pada ambang PT 1% (Rp 50.000)",
    ...
    ok(cfg_set(admin, "purchasing.bill_price_tolerance_percent", 10, "entity", ENT_A),
       "toleransi harga dinaikkan ke 10% lewat Pusat Pengaturan (scope PT)")
    ...
    ok(cfg_reset(admin, "purchasing.bill_price_tolerance_percent", "entity", ENT_A),
...
def cleanup(admin: str, stock_snap: Any) -> None:
    ...
    async def purge(db):
        ...
        await db.system_settings.update_one(
            {"scope": ENT_A}, {"$unset": {"contra_bon": "", "purchasing.bill_price_tolerance_percent": ""}})
...
def main() -> int:
    print(f"{B}{C}{'=' * 78}\n  POC FASE G-7 — KONTRABON ADVANCED (tukar faktur supplier)\n"
          ...
          f"{'=' * 78}{X}")
    # Sidik jejak audit/notifikasi DIAMBIL SEBELUM login (login menulis audit_logs).
    from poc_stock_guard import snapshot_new_ids, purge_new_ids
    journal = snapshot_new_ids()
    ...
    # POC-RESIDU-04 — POC ini menulis toleransi kontrabon PER BADAN USAHA (itu
    # memang yang diujinya: "toleransi datang dari Pusat Pengaturan"), dan
    # penulisan itu MELAHIRKAN dokumen scope `ent_ksc` di `system_settings` yang
    # tak pernah dihapus. Terukur: satu kali gate → `system_settings` 4 → 5.
    # Konfigurasi adalah SSOT perilaku, jadi ikut dipulihkan EKSAK.
    stock = snapshot_stock(STOCK_COLLECTIONS + ["system_settings"])
    cb_id = ""
    try:
        ctx = setup(admin)
        test_schedule(admin)
    ...
    from poc_stock_guard import prune_orphan_turn_marks as _prune_turn
    _prune_turn(verbose=False)
    purge_new_ids(journal, verbose=False)
