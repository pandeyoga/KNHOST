...
async def cleanup(db):
    ...
    # (`inspection_service.ensure_for_qc_task`, idempotent). POC ini menerima barang PO,
    # jadi SPK itu adalah AKIBAT dari pekerjaannya dan ikut jadi tanggung jawabnya.
    #
    # TERUKUR 2026-08-23 — kenapa baris ini ada: tanpa dia `inspections` tumbuh **+1
    # setiap `gate.sh --full`** (3 → 4 → 5 …), dan karena SPK-nya menyimpan ref ke PO
    # yang POC ini hapus, `audit_doc_refs` memerah **"Tautan menggantung: 1 temuan"** di
    # gate BERIKUTNYA. Gejalanya muncul jauh dari sebabnya: yang terbaca merah adalah
    # **POC P-0** dan **POC FASE P** (keduanya memanggil INV-REF), bukan POC ini —
    # persis kelas "gema" yang sudah memakan waktu satu sesi penuh (pelajaran FASE S).
    ins = await db.inspections.find({"$or": [{"ref_doc_id": {"$in": po_ids}},
                                             {"task_id": {"$in": task_ids}}]},
                                    {"_id": 0, "id": 1}).to_list(500)
    ins_ids = [x["id"] for x in ins]
    if ins_ids:
        await db.inspections.delete_many({"id": {"$in": ins_ids}})
        # Acuan sample yang di-ACC ditaut DUA ARAH oleh `_new_doc`, jadi menghapus SPK
        # saja meninggalkan jejak di `md_samples` yang menunjuk ketiadaan.
...
def main():
    ...
    print("=" * 64)
    # POC-RESIDU-04 — POC ini menyalakan kebijakan penerimaan (`receiving.*`), dan
    # penulisan itu MELAHIRKAN dokumen scope `receiving` di `system_settings` yang
    # tak pernah dihapus. Terukur: satu kali gate → `system_settings` 3 → 4.
    # Konfigurasi adalah SSOT perilaku aplikasi, jadi dipulihkan EKSAK.
    _cfg_snap = snapshot_stock(["system_settings"])
    try:
        asyncio.run(run())
...
if __name__ == "__main__":
    __import__('poc_stock_guard').with_journal_cleanup(main)
