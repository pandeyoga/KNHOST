...
async def wipe(db):
    ...
    if audit_ids:
        ...
        # Tanda "giliran sudah diberi tahu" (turn_marks) & riwayat revaluasi roll
        # (roll_cost_history) ikut lahir dari dokumen POC — terukur +4/+1 oleh
        # ukur_residu_poc.py; dibersihkan supaya POC benar-benar nol residu.
        await db.turn_marks.delete_many({"doc_id": {"$in": audit_ids}})
    if roll_ids:
        await db.roll_cost_history.delete_many({"roll_id": {"$in": roll_ids}})
    ...
    # FASE I — barang yang masuk antrean QC MELAHIRKAN SPK inspeksi otomatis
    # (`inspection_service.ensure_for_qc_task`). Rantai POC ini menerima barang PO di
    # dua badan usaha, jadi SPK-nya lahir dan ikut menjadi tanggung jawabnya.
    #
    # TERUKUR 2026-08-23: tanpa baris ini POC meninggalkan `KANDA/INS-00002` setiap
    # jalan, lalu ref SPK→PO (PO-nya dihapus di bawah) membuat INV-REF memerah
    # **"Tautan menggantung: 1 temuan"** di gate — dan yang terbaca merah bukan POC ini,
    # melainkan **POC P-0** & **POC FASE P** (dua POC pertama sesudahnya yang memanggil
    # INV-REF), plus 8 POC FASE G berikutnya. Sebelas gate merah, satu sebab.
    ins_ids = [x["id"] for x in await db.inspections.find(
        {"$or": [{"ref_doc_id": {"$in": po_ids + sret_ids + pret_ids}},
                 {"task_id": {"$in": task_ids}}]}, {"_id": 0, "id": 1}).to_list(500)]
    if ins_ids:
        await db.inspections.delete_many({"id": {"$in": ins_ids}})
        await db.md_samples.update_many(
            {"refs.doc_id": {"$in": ins_ids}},
            {"$pull": {"refs": {"doc_id": {"$in": ins_ids}}}})
...
def step4_customer_return(adm_a, mgr_a):
    ...
    # T1 audit training 2026-06: gerbang verifikasi Admin Sales kini bawaan HIDUP,
    # jadi urutannya = dokumen: ajukan → ACC manajer → verifikasi → konfirmasi.
    for path in ("submit-for-approval", "approve", "verify", "confirm"):
        rr = adm_a.post(f"{API}/sales-orders/{so['id']}/{path}",
                        json={"notes": f"{MARK} {path}", "note": f"{MARK} {path}"}, timeout=60)
...
async def run():
    ...
    # POC-RESIDU-04 — langkah "retur diselesaikan dengan KREDIT TOKO" menulis satu
    # baris `store_credit_ledger`. Itu SALDO UANG pelanggan: kalau tertinggal, saldo
    # kredit pelanggan demo naik tiap kali POC/gate dijalankan, dan POC lain yang
    # menguji pengembalian kredit bisa lulus/gagal karena saldo warisan itu.
    ST["credit_snap"] = snapshot_stock(["store_credit_ledger"])
    ...
    try:
        ...
        post = await wipe(db)
        restore_stock(ST.get("credit_snap"))
