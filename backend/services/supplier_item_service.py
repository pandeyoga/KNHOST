...
# 2. CRUD
# ═══════════════════════════════════════════════════════════════════════════
def _clean_label_pattern(raw: Any) -> Optional[Dict[str, Any]]:
    """FASE SL — pola label per Barang Supplier; None/{} = ikut pola supplier."""
    if not isinstance(raw, dict) or not raw:
        return None
    from services import label_decoder_service as ldec
    return ldec.normalize_pattern(raw)


async def label_pattern_candidates(*, supplier_id: str, product_id: str,
                                   entity_id: str = "") -> List[Dict[str, Any]]:
    """Barang Supplier aktif (supplier × produk) yang punya pola label sendiri — dicoba lebih dulu saat scan."""
    if not (supplier_id and product_id):
        return []
    flt: Dict[str, Any] = {"supplier_id": supplier_id, "product_id": product_id, "status": "active",
                           "label_pattern": {"$type": "object"}}
    if entity_id:
...
async def create_item(payload: Dict[str, Any], *, entity_id: str,
    ...
    doc: Dict[str, Any] = {
        ...
        "status": status,
        "label_pattern": _clean_label_pattern(payload.get("label_pattern")),
        "usage_count": 0,
...
async def patch_item(sid: str, payload: Dict[str, Any], actor: str = "") -> Dict[str, Any]:
    ...
    if "label_pattern" in payload and payload["label_pattern"] is not None:
        upd["label_pattern"] = _clean_label_pattern(payload["label_pattern"])  # {} → None = ikut supplier
    if not upd:
        return safe_doc(cur)
