...
async def move_rolls_by_qty(product_id: str, warehouse_id: str, owner_entity_id: str,
    ...
    for roll in rolls:
        ...
        # R-4 — CAS: prasyarat status + sisa panjang; roll yang sudah dipindah/dipotong
        # proses lain TIDAK dipindah dua kali, melainkan dilewati (FEFO lanjut ke roll berikut).
        cas = {"id": roll["id"], "status": from_status, "length_remaining": roll["length_remaining"]}
        if rlen <= remaining + 0.01:
            res = await db.inventory_rolls.update_one(
                cas, {"$set": {"status": to_status, "bucket_ref": ref, "updated_at": now_iso()}})
            ...
            res = await db.inventory_rolls.update_one(
                cas, {"$set": {"length_remaining": round(rlen - remaining, 2),
 ...
 "qty_rolls": (1 if mid else None),
    ...
    if remaining > 0.01 and conflicts:
        # Kompensasi: roll yang sudah terpindah dikembalikan agar tidak ada hold/WIP setengah jadi.
        if moved and (ref or {}).get("id"):
            await move_rolls_by_ref(ref["id"], from_status, movement_type=f"{movement_type}_rollback")
        raise HTTPException(
            status_code=409,
            detail=f"Stok '{from_status}' berubah di proses lain ({conflicts} roll terlewati). Muat ulang lalu coba lagi.")
    return {"moved": round(qty - max(remaining, 0), 2), "rolls": moved, "conflicts": conflicts}
