...
# ─── CRUD ──────────────────────────────────────────────────────
async def _assert_design_not_held(design_id: str, stypes: List[str]) -> None:
    """Desain yang di-HOLD (Design Studio) tidak boleh masuk proofing R&D."""
    if not design_id or "proofing" not in [str(t).lower() for t in stypes or []]:
        return
    row = await db.design_gallery.find_one({"id": design_id}, {"_id": 0, "on_hold": 1, "code": 1, "hold": 1})
    if row and row.get("on_hold"):
        h = row.get("hold") or {}
        raise RndError(f"Desain '{row.get('code') or design_id}' sedang HOLD"
                       + (f" oleh {h.get('by')} — {h.get('reason')}" if h.get("by") else "")
                       + ". Lepas hold di Design Studio dulu sebelum masuk proofing.")


async def _log_design(sample: Dict[str, Any], actor: Any, kind: str, note: str, **extra) -> None:
    if not sample.get("design_id"):
        return
    from services import design_studio_service as _studio
    await _studio.log_external(sample["design_id"], actor, kind, note, sample_id=sample.get("id", ""),
                               sample_number=sample.get("number", ""), **extra)


def _is_proofing_sample(sample: Dict[str, Any]) -> bool:
    return "proofing" in [str(t).lower() for t in types_of(sample)]
...
async def create_sample(payload: Dict[str, Any], *, entity_id: str,
    ...
    await _assert_design_not_held(design_id, stypes)
    doc: Dict[str, Any] = {
        "id": new_id(PREFIX),
    ...
    await _log_design(doc, actor, "proofing_requested" if "proofing" in stypes else "sample_requested",
                      f"{doc['number']} — {doc['title']} ({' + '.join(stypes)})", sample_types=stypes,
                      sample_status="draft")
    return safe_doc(doc)
...
def spec_summary(spec: Dict[str, Any], template_name: str = "") -> Dict[str, Any]:
    t = spec.get("target") or {}
    return {"id": spec.get("id"), "number": spec.get("number"), "status": spec.get("status"),
            "fabric_type": t.get("fabric_type", ""), "gramasi": t.get("gramasi"), "lebar": t.get("lebar"),
            "template_id": spec.get("template_id", ""), "template_name": template_name,
            "sku_hint": spec.get("sku_hint", ""), "product_sku": spec.get("product_sku", "")}


async def attach_spec_summaries(rows: List[Dict[str, Any]]) -> None:
    """Kartu daftar: ringkas spesifikasi (induk · jenis kain · gramasi · lebar) tanpa membuka rincian."""
    ids = [r.get("spec_id") for r in rows if r.get("spec_id")]
    if not ids:
        return
    specs_ = {s["id"]: s for s in await db.md_specs.find({"id": {"$in": ids}}, {"_id": 0, "id": 1, "number": 1, "status": 1, "target": 1,
                                                                                    "template_id": 1, "sku_hint": 1, "product_sku": 1}).to_list(len(ids))}
    tpl_ids = list({s.get("template_id") for s in specs_.values() if s.get("template_id")})
    tpls = {t["id"]: t.get("name", "") for t in await db.product_templates.find({"id": {"$in": tpl_ids}}, {"_id": 0, "id": 1, "name": 1}).to_list(len(tpl_ids))} if tpl_ids else {}
    for r in rows:
        s = specs_.get(r.get("spec_id"))
        r["spec_summary"] = spec_summary(s, tpls.get(s.get("template_id"), "")) if s else None


async def link_spec(sample_id: str, spec: Dict[str, Any], *, actor: str = "") -> Dict[str, Any]:
    await db[COLL].update_one({"id": sample_id}, {"$set": {
        "spec_id": spec["id"], "spec_number": spec.get("number", ""), "updated_at": now_iso()}})
    await db.md_specs.update_one({"id": spec["id"]}, {"$addToSet": {"sample_ids": sample_id}})
    doc = await get_sample(sample_id)
    await _log_design(doc, actor, "spec_linked", f"Spesifikasi {spec.get('number')} dilengkapi untuk {doc.get('number')}")
    return doc


async def master_data_of(doc: Dict[str, Any]) -> Dict[str, Any]:
    """Ringkasan master data yang lahir dari permintaan sample ini (untuk panel 'Master data' di rincian)."""
    dec = doc.get("decision") or {}
    spec = doc.get("spec") or {}
    out: Dict[str, Any] = {"spec": None, "product": None, "template": None, "contract": None,
                           "supplier_item": None, "color": None, "decided": bool(dec.get("supplier_id"))}
    if spec:
        out["spec"] = {"id": spec.get("id"), "number": spec.get("number"), "status": spec.get("status"),
                       "lifecycle": spec.get("lifecycle"), "title": spec.get("title"),
                       "sku_hint": spec.get("sku_hint", ""), "template_id": spec.get("template_id", ""),
                       "target": spec.get("target") or {}, "base_unit": spec.get("base_unit", "")}
    product_id = dec.get("product_id") or spec.get("product_id") or doc.get("product_id") or ""
    if product_id:
        prod = await db.products.find_one({"id": product_id}, {"_id": 0, "id": 1, "sku": 1, "name": 1,
                                                                "lifecycle": 1, "template_id": 1,
                                                                "variant_attrs": 1, "variant_options": 1,
                                                                "color_ref": 1, "supplier_colors": 1, "rnd_supplier": 1})
        if prod:
            out["product"] = prod
    template_id = (out["product"] or {}).get("template_id") or spec.get("template_id") or ""
    if template_id:
        tpl = await db.product_templates.find_one({"id": template_id}, {"_id": 0, "id": 1, "name": 1, "code": 1, "sku_prefix": 1})
        if tpl:
            out["template"] = tpl
    if dec.get("contract_id"):
        out["contract"] = {"id": dec["contract_id"], "number": dec.get("contract_number", ""), "price": dec.get("price")}
    if dec.get("supplier_item_id"):
        out["supplier_item"] = {"id": dec["supplier_item_id"], "supplier_sku": dec.get("supplier_sku", ""),
                                "supplier_name": dec.get("supplier_name", "")}
    color_id = (doc.get("color_target") or {}).get("color_id") or ""
    if color_id:
        col = await db.color_library.find_one({"id": color_id}, {"_id": 0, "id": 1, "code": 1, "name": 1, "hex": 1,
...
async def patch_sample(sample_id: str, payload: Dict[str, Any], actor: str = "") -> Dict[str, Any]:
    ...
    if payload.get("design_id") is not None:
        ...
        await _assert_design_not_held(data.get("design_id", ""), types_of(cur))
    ...
    if payload.get("sample_types") is not None:
        ...
        await _assert_design_not_held(data.get("design_id", cur.get("design_id", "")), want)
...
async def finish_sample(sample_id: str, payload: Dict[str, Any],
    ...
    await _log_design(cur, actor, "proofing_finished" if _is_proofing_sample(cur) else "sample_finished",
                      f"{cur.get('number')} ditandai JADI ({str(at)[:10]})" + (f" — {note}" if note else ""),
                      sample_status=cur.get("status"), finished_at=str(at)[:10])
    return await get_sample(sample_id)
...
async def decide_sample(sample_id: str, payload: Dict[str, Any],
    ...
    lock_target = cur.get("exclusive_customer_id") or ""
    if not lock_target and cur.get("spec_id"):
        lock_target = ((await db.md_specs.find_one({"id": cur["spec_id"]}, {"_id": 0, "exclusive_customer_id": 1})) or {}).get("exclusive_customer_id") or ""
    await specs._assert_color_lock(cur.get("color_target") or {}, lock_target, f"sample {cur.get('number', '')}")  # noqa: SLF001
    snap = await _supplier_snapshot(supplier_id)
    name = (actor or {}).get("name", "")
    ...
    decision = {
        ...
        "supplier_color_name": (payload.get("supplier_color_name") or "").strip(),
        "supplier_color_code": (payload.get("supplier_color_code") or "").strip(),
        "round_id": acc[-1].get("id", ""), "round_no": acc[-1].get("round_no"),
        "score": acc[-1].get("score"),
    }
    # Warna versi supplier → disinkronkan ke pustaka warna (satu warna, banyak versi supplier).
    color_id = (cur.get("color_target") or {}).get("color_id") or ""
    if color_id and (decision["supplier_color_name"] or decision["supplier_color_code"]):
        variant = {"supplier_id": supplier_id, "supplier_name": snap["supplier_name"],
                   "supplier_color_name": decision["supplier_color_name"],
                   "supplier_color_code": decision["supplier_color_code"],
                   "sample_id": sample_id, "sample_number": cur.get("number", ""), "at": now_iso()}
        await db.color_library.update_one({"id": color_id}, {"$pull": {"supplier_variants": {"supplier_id": supplier_id}}})
        upd = {"$push": {"supplier_variants": variant}, "$set": {"updated_at": now_iso()}}
        col = await db.color_library.find_one({"id": color_id}, {"_id": 0, "factory_name": 1})
        if col is not None and not (col.get("factory_name") or "").strip():
            upd["$set"]["factory_name"] = decision["supplier_color_name"] or decision["supplier_color_code"]
        await db.color_library.update_one({"id": color_id}, upd)
        decision["color_synced"] = True
    # Sekaligus ACC spesifikasi → master produk lahir (bila diminta & spesifikasi masih terbuka).
    if payload.get("approve_spec") and cur.get("spec_id"):
        spec_doc = await db.md_specs.find_one({"id": cur["spec_id"]}, {"_id": 0, "status": 1, "product_id": 1, "product_sku": 1})
        if spec_doc and spec_doc.get("status") in ("draft", "review"):
            try:
                res = await specs.approve_spec(cur["spec_id"], {
                    "sku": payload.get("product_sku") or "", "name": payload.get("product_name") or "",
                    "price": 0, "note": f"ACC bersama keputusan sample {cur.get('number')}"}, actor)
                decision["product_id"] = (res.get("product") or {}).get("id", "")
                decision["product_sku"] = (res.get("product") or {}).get("sku", "")
            except RndError as exc:
                decision["master_error"] = str(exc)
        elif spec_doc:
            decision["product_id"] = spec_doc.get("product_id", "")
            decision["product_sku"] = spec_doc.get("product_sku", "")
    # Dua warna + supplier pemenang → tersalin ke master produk (dibaca MD, Pembelian, pemilih produk, katalog).
    if cur.get("spec_id"):
        from services import color_service as _colors
        spec_now = await db.md_specs.find_one({"id": cur["spec_id"]}, {"_id": 0, "product_id": 1})
        pid = decision.get("product_id") or (spec_now or {}).get("product_id") or ""
        if pid:
            await db[COLL].update_one({"id": sample_id}, {"$set": {"decision": decision}})
            await _colors.sync_product_colors(pid)
            # Kontrak pemenang lahir sebelum produk → ikat ke produk agar sourcing PO memilih kontrak yang benar.
            if decision.get("contract_id"):
                await db.supplier_contracts.update_one({"id": decision["contract_id"], "product_id": {"$in": ["", None]}},
                                                       {"$set": {"product_id": pid, "updated_at": now_iso()}})
            if cur.get("exclusive_customer_id"):
    ...
    # Penanda Warna Otomatis — warna baru dari pesanan khusus jadi milik pelanggan pemesan saat labdip disetujui.
    if cur.get("exclusive_customer_id") and color_id:
        from services import color_lock_service as _lock
        cname = cur.get("customer_name") or ((await db.customers.find_one({"id": cur["exclusive_customer_id"]}, {"_id": 0, "name": 1})) or {}).get("name", "")
        if await _lock.auto_mark_owner(color_id, cur["exclusive_customer_id"], cname, {
                "sample_id": sample_id, "sample_number": cur.get("number", ""),
                "special_order_number": cur.get("special_order_number", ""), "by": name}):
            decision["color_auto_owned"] = True
            decision["color_owner_name"] = cname
    await db[COLL].update_one({"id": sample_id}, {
    ...
    await _log_design(cur, actor, "proofing_decided" if _is_proofing_sample(cur) else "sample_decided",
                      f"{cur.get('number')} diputus — pemenang {snap['supplier_name']}"
                      + (f" · kontrak {decision['contract_number']}" if decision["contract_number"] else ""),
                      sample_status="decided", supplier_name=snap["supplier_name"], price=price)
    return await get_sample(sample_id)
