...
async def save_product(product, *, actor="System", existing=None, entity_id="", strict=True):
    ...
    if doc.get("color_code") and (not existing or existing.get("color_code") != doc.get("color_code")
                                  or (existing.get("exclusive_customer_id") or "") != (doc.get("exclusive_customer_id") or "")):
        from services import color_lock_service as _lock
        try:
            await _lock.assert_can_use(code=doc["color_code"], target_customer_id=doc.get("exclusive_customer_id") or "",
                                       context=f"produk {doc.get('sku', '')}")
