...
async def sales_queues(actor, scope, ids) -> List[Dict[str, Any]]:
    ...
    waiting = await _find("sales_orders", {**sq, **mine, "status": {"$in": ["waiting_approval", "waiting_stock"]}})
    approved = await _find("sales_orders", {**sq, **mine, "status": "approved"})
    rejected = await _find("sales_orders", {**sq, **mine, "status": {"$in": ["rejected", "cancelled"]}, "updated_at": {"$gte": _days_ago(7)}})
    samples = await _find("sample_requests", {**sq, "$or": [{"requested_by_id": actor["id"]}, {"requested_by": actor.get("name")}], "status": {"$in": ["requested", "done"]}})
    return [
        ...
        _queue("sampel_saya", "Sampel saya", "Sampel yang sedang disiapkan atau siap diserahkan.",
               [_row(ref_type="sample_request", ref_id=o["id"], number=o.get("number", ""), title=o.get("customer_name", "—"), subtitle=o.get("product_name", ""),
                     age_days=_age_days(o.get("created_at")), badge=o.get("status", ""), action="Lihat") for o in samples],
               action_label="Lihat", owner="sales", value_kind="count", value_label="Item"),
    ]


async def warehouse_queues(scope, ids) -> List[Dict[str, Any]]:
    sq = _scope_q(scope, ids)
    tasks = await _find("wms_tasks", {**sq, "status": {"$in": ["created", "pending", "picking", "packing"]}})
    po = await _find("purchase_orders", {**sq, "status": {"$in": ["pending", "receiving"]}})
    samples = await _find("sample_requests", {**sq, "status": "requested"})
    return [
        ...
        _queue("sampel_potong", "Sampel untuk dipotong", "Permintaan sampel dari sales.",
               [_row(ref_type="sample_request", ref_id=o["id"], number=o.get("number", ""), title=o.get("customer_name", "—"), subtitle=o.get("product_name", ""),
                     age_days=_age_days(o.get("created_at")), badge=o.get("status", ""), action="Potong") for o in samples],
...
async def designer_queues(actor, scope, ids) -> List[Dict[str, Any]]:
    sq = _scope_q(scope, ids)
    mine = await _find("design_requests", {**sq, "assigned_to": actor["id"], "status": {"$in": ["approved", "in_progress"]}})
    unassigned = await _find("design_requests", {**sq, "status": "approved", "$or": [{"assigned_to": ""}, {"assigned_to": None}]})
