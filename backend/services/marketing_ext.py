...
# ─── Template konten & duplikasi post ────────────────────────────────────────
TEMPLATE_FIELDS = ("title", "caption", "hashtags", "platforms", "content_type", "cta", "notes")


async def create_template(payload: Dict[str, Any], *, entity_id: str, actor: Dict[str, Any]) -> Dict[str, Any]:
    from services.marketing_service import CONTENT_TYPES, _hashtags
    name = (payload.get("name") or "").strip()
    if not name:
        raise MarketingError("Nama template wajib diisi.")
    if await db.mkt_templates.find_one({"entity_id": entity_id, "name": {"$regex": f"^{_rx(name)}$", "$options": "i"}}, {"_id": 1}):
        raise MarketingError(f"Template '{name}' sudah ada di badan usaha ini.")
    platforms = [p for p in (payload.get("platforms") or []) if p in {x["code"] for x in PLATFORMS}]
    ctype = payload.get("content_type") or "foto"
    if ctype not in CONTENT_TYPES:
        raise MarketingError("Jenis konten tidak dikenal.")
    doc = {"id": new_id("mkt"), "entity_id": entity_id, "name": name, "title": (payload.get("title") or "").strip(),
           "caption": (payload.get("caption") or "").strip(), "hashtags": _hashtags(payload.get("hashtags")),
           "platforms": platforms, "content_type": ctype, "cta": (payload.get("cta") or "").strip(),
           "notes": (payload.get("notes") or "").strip(), "source_post_id": payload.get("source_post_id") or "",
           "used_count": 0, "created_by": actor.get("name", ""), "created_by_id": actor.get("id", ""),
           "created_at": now_iso(), "updated_at": now_iso()}
    await db.mkt_templates.insert_one(dict(doc))
    return doc
...
async def list_templates(scope: Dict[str, Any]) -> List[Dict[str, Any]]:
    return await db.mkt_templates.find(scope, {"_id": 0}).sort([("used_count", -1), ("name", 1)]).to_list(300)


async def mark_template_used(tid: str) -> None:
    await db.mkt_templates.update_one({"id": tid}, {"$inc": {"used_count": 1}})


def duplicate_payload(src: Dict[str, Any], *, same_entity: bool, publish_at: str = "") -> Dict[str, Any]:
    """Isi post baru dari post sumber: konten disalin, status/lampiran/performa/riwayat TIDAK."""
    return {"title": f"Salinan · {src.get('title', '')}"[:200], "caption": src.get("caption", ""),
            "hashtags": src.get("hashtags") or [], "platforms": src.get("platforms") or [],
            "content_type": src.get("content_type") or "foto", "publish_at": (publish_at or "").strip(),
            "campaign_id": src.get("campaign_id", "") if same_entity else "",
            "pic_user_id": src.get("pic_user_id", ""), "pic_name": src.get("pic_name", ""),
            "cta": src.get("cta", ""), "notes": src.get("notes", ""), "assets": src.get("assets") or [],
            "account_ids": [a["id"] for a in src.get("accounts") or []] if same_entity else [],
            "status": "idea"}
