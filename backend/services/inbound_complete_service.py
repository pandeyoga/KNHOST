"""Penyelesaian penerimaan tugas inbound — SATU-SATUNYA jalur yang memposting stok, `received_qty` PO,
HPP, dan jurnal penerimaan (GRN Fase 1: dipindah apa adanya dari `routers/inbound_receiving.py`).

Router lama menjadi pembungkus tipis; layanan GRN (Fase 2) memanggil `complete_task(..., via_grn=grn)`.
"""
...
from entity_scope import EntityContext, assert_entity_access
from schemas import GRCompletePayload


def grade_or_400(raw: Any, default: str = "A") -> str:
    """Grade masukan → enum SSOT (A+→A, C→BS); tak dikenal → 400."""
    try:
...
_grade_or_400 = grade_or_400


async def complete_task(task_id: str, payload: Optional[GRCompletePayload], actor: Dict[str, Any], *,
                        ctx: Optional[EntityContext] = None,
                        via_grn: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Complete inbound receiving and update inventory (qc_check/put_away → completed | qc_pending).

    P0-4 — `payload.rolls` diisi → multi-roll; kosong → roll hasil scan label, atau satu roll default.
    `ctx` diisi router (cek entitas); `via_grn` diisi layanan GRN.
    """
    ...
    if ctx is not None:
        assert_entity_access(task, "wms_tasks", ctx)
    
    # `via_grn` (Fase 2): GRN boleh menutup tugas `receiving` (kiriman parsial sah, tanpa eskalasi).
    if task["status"] not in ["qc_check", "put_away"] + (["receiving"] if via_grn else []):
        raise HTTPException(
            status_code=400,
    ...
    _receipt_variance: Dict[str, Any] = {}
    if next_stage in ("completed", "qc_pending"):
        ...
        # FASE SL — roll yang SUDAH lahir dari scan label supplier (status `receiving`)
        # adalah SSOT: GR tidak melahirkan roll baru, melainkan MEMFINALKAN roll itu.
        _scanned_rolls = await db.inventory_rolls.find(
            {"grn_task_id": task_id, "status": "receiving"}, {"_id": 0}).sort("created_at", 1).to_list(500)
        roll_specs: List[Dict[str, Any]] = []
        if _scanned_rolls:
            for r in _scanned_rolls:
                roll_specs.append({
                    "existing": r,
            ...
            final_qty = round(sum(float(r.get("actual_task_qty") if r.get("actual_task_qty") is not None
                                        else r.get("declared_task_qty") or 0) for r in _scanned_rolls), 2)
            if final_qty <= 0:
                final_qty = task.get("received_qty", 0.0)
        elif payload and payload.rolls:
        ...
        if _variance.get("level") == "block" and _scanned_rolls:
            # FASE SL — berat roll hasil scan sudah DIKONFIRMASI per roll terhadap label
            # (toleransi label vs aktual → flag QC). Selisih vs konversi GSM×lebar tetap
            # DICATAT (needs_review) tetapi tidak memblokir gudang.
            _variance["level"] = "warn"
        ...
        # ── FASE C (D-10/D-26/D-27) — pagar LOT (400 mode `block`) SEBELUM klaim ──────
        # Sesi 4 (2026-09): dulu pagar ini di bawah klaim → 400 meninggalkan `saga_lock`
        # dan percobaan ulang yang sudah benar ditolak 409 SAGA_IN_PROGRESS (bukti:
        # test_fase_c_lot_poc TEST 3). Klaim wajib SESUDAH semua validasi 400.
        ...
        _supplier_lot = ((payload.supplier_lot if payload else "") or
                         task.get("supplier_lot") or
                         next((r.get("supplier_lot") for r in _scanned_rolls if r.get("supplier_lot")), "") or
                         "").strip()
        ...
        await _saga.claim("wms_tasks", task_id, "inbound_complete",
                          precondition={"status": {"$in": ["qc_check", "put_away"] + (["receiving"] if via_grn else [])}},
                          actor=actor["name"])
        ...
        if _po_doc:
            for _it in _po_doc.get("items", []):
                if _it.get("product_id") == task["product_id"]:
                    # KN-A09 — HPP memakai harga NETO baris (line_total ÷ qty), bukan `price`
                    # bruto: diskon pembelian ikut menurunkan nilai persediaan. Diskon header
                    # PO dibagi proporsional ke tiap baris.
                    _q = float(_it.get("quantity", _it.get("qty", 0)) or 0)
                    _lt = _it.get("line_total")
                    if _lt is not None and _q > 0:
                        _po_unit_price = float(_lt or 0) / _q
                    else:
                        _po_unit_price = float(_it.get("price", 0) or 0) * (
                            1 - float(_it.get("discount_percent", 0) or 0) / 100.0)
                    _hdr_pct = float(_po_doc.get("order_discount_percent", 0) or 0)
                    if _hdr_pct > 0:
                        _po_unit_price *= (1 - _hdr_pct / 100.0)
                    _po_unit_price = round(_po_unit_price, 4)
                    break
        if _po_unit_price <= 0:
        ...
        # FASE SL — CATATAN SELISIH vs PO (roll & qty): dibaca purchasing + finance.
        _exp_qty = float(task.get("expected_qty") or 0)
        _exp_rolls = task.get("expected_rolls")
        _diff_qty = round(final_qty - _exp_qty, 2) if _exp_qty > 0 else 0.0
        _diff_rolls = (len(roll_specs) - int(_exp_rolls)) if _exp_rolls not in (None, "", 0) else None
        _flagged_rolls = [s["existing"].get("roll_no") for s in roll_specs
                          if s.get("existing") and (s["existing"].get("label_variance") or {}).get("flagged")]
        _receipt_variance = {}
        if _diff_qty or _diff_rolls or _flagged_rolls:
            _receipt_variance = {
                "expected_qty": _exp_qty, "received_qty": final_qty, "diff_qty": _diff_qty,
                "diff_pct": round(_diff_qty / _exp_qty * 100, 2) if _exp_qty > 0 else None,
                "unit": gr_task_unit, "expected_rolls": _exp_rolls, "received_rolls": len(roll_specs),
                "diff_rolls": _diff_rolls, "label_variance_rolls": _flagged_rolls,
                "note": " · ".join(filter(None, [
                    (f"qty {'lebih' if _diff_qty > 0 else 'kurang'} {abs(_diff_qty):g} {gr_task_unit}" if _diff_qty else ""),
                    (f"roll {'lebih' if _diff_rolls > 0 else 'kurang'} {abs(_diff_rolls)}" if _diff_rolls else ""),
                    (f"{len(_flagged_rolls)} roll selisih label vs aktual" if _flagged_rolls else ""),
                ])), "po_number": task.get("po_number", ""), "task_id": task_id,
                "recorded_at": now_iso(), "recorded_by": actor["name"],
            }
            if task.get("po_id"):
                await db.purchase_orders.update_one(
                    {"id": task["po_id"]}, {"$push": {"receipt_variances": _receipt_variance}})
        ...
        for spec in roll_specs:
            ...
            _ex = spec.get("existing") or {}
            roll_doc = {
                "id": _ex.get("id") or new_id("roll"),
                "product_id": task["product_id"],
                "owner_entity_id": owner_entity_id,
                "ownership_type": "internal",
                "consignor_ref": None,
                "warehouse_id": task["warehouse_id"],
                "bin_id": _ex.get("bin_id") or task.get("bin_id") or None,
                "lot": _lot_doc.get("lot_number") or lot,
                "lot_id": _lot_doc.get("id") or "",
                "supplier_lot": _ex.get("supplier_lot") or _supplier_lot,
                "dye_lot": spec["dye_lot"],
                "batch": task.get("batch") or (lot.replace("LOT", "BATCH") if lot else ""),
                "roll_no": _ex.get("roll_no") or await _next_roll_no(),
            ...
            if _ex:
                # FASE SL — finalkan roll hasil scan: identitas supplier/deklarasi/alias TETAP,
                # hanya field stok (status, lot, HPP, jejak) yang ditulis.
                _keep = {"id", "roll_no", "created_at", "created_by", "created_by_name", "rfid_tag_id",
                         "tracking_mode", "received_date", "acquired"}
                _final = {k: v for k, v in roll_doc.items() if k not in _keep}
                _final["journey"] = {"stage": "received_transit", "routing": "store", "updated_at": now_iso()}
                await db.inventory_rolls.update_one({"id": _ex["id"]}, {"$set": _final})
                roll_doc = {**_ex, **_final}
            else:
        ...
        if task.get("po_id"):
            # KN-D01 — jangan menghidupkan kembali PO terminal (cancelled/closed_short/
            # completed) menjadi 'receiving'; received_qty tetap dicatat, status hanya
            # bergeser bila PO memang masih terbuka (recompute_po_status menutupnya lagi).
            _TERMINAL_PO = ["cancelled", "closed_short", "completed", "rejected"]
            _po_pred = {"id": task["po_id"], "items.product_id": task["product_id"]}
            _res = await db.purchase_orders.update_one(
                {**_po_pred, "status": {"$nin": _TERMINAL_PO}},
                {
            ...
            if _res.matched_count == 0:
                await db.purchase_orders.update_one(
                    _po_pred,
                    {"$inc": {"items.$.received_qty": final_qty},
                     "$set": {"last_received_at": now_iso(), "updated_at": now_iso()}})
    ...
    updated_task = await db.wms_tasks.find_one_and_update(
        ...
        {"$set": {
            ...
            **({"receipt_variance": _receipt_variance, "needs_review": True} if _receipt_variance else {}),
    ...
    if _receipt_variance:
        out["receipt_variance"] = _receipt_variance
