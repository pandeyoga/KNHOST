...
async def _assert_color_lock(color: Dict[str, Any], exclusive_customer_id: str, context: str = "spesifikasi") -> None:
    """Kunci Warna Eksklusif — warna milik pelanggan lain butuh izin manager."""
    from services import color_lock_service as _lock
    try:
        await _lock.assert_can_use(color_id=(color or {}).get("color_id") or "",
                                   target_customer_id=exclusive_customer_id, context=context)
    except _lock.ColorLockError as exc:
...
async def create_spec(payload: Dict[str, Any], *, entity_id: str, actor: str = "") -> Dict[str, Any]:
    ...
    await _assert_color_lock(color, payload.get("exclusive_customer_id") or "")
...
async def patch_spec(spec_id: str, payload: Dict[str, Any], actor: str = "") -> Dict[str, Any]:
    ...
    if payload.get("color_target") is not None:
        ...
        await _assert_color_lock(data["color_target"], cur.get("exclusive_customer_id") or "")
