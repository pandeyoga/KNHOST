...
async def list_customer_colors(visible_customer_ids: Optional[set] = None, customer_id: str = "") -> List[Dict[str, Any]]:
    ...
    def slot(cid: str, cname: str, color: Dict[str, Any]) -> Dict[str, Any]:
        ...
        u = usage.get(color["id"]) or {}
        reqs = color.get("use_requests") or []
        return g["colors"].setdefault(color["id"], {
            **{k: v for k, v in color.items() if k not in ("supplier_variants", "use_requests")},
            ...
            "supplier_versions": [{"supplier_name": v.get("supplier_name", ""), "supplier_color_name": v.get("supplier_color_name", ""),
                                   "supplier_color_code": v.get("supplier_color_code", "")} for v in color.get("supplier_variants") or []],
            "grants": [{"id": r["id"], "target_customer_id": r.get("target_customer_id", ""),
                        "target_customer_name": r.get("target_customer_name", "")} for r in reqs if r.get("status") == "approved"],
            "pending_requests_count": sum(1 for r in reqs if r.get("status") == "pending")})
...
async def create_color(data: Dict[str, Any], actor_name: str = "") -> Dict[str, Any]:
    ...
    doc = {
        ...
        **(await _customer_owner(data.get("exclusive_customer_id") or "")),
        "exclusive_source": {"kind": "manual", "at": now_iso()} if data.get("exclusive_customer_id") else {},
        "status": "active",
...
async def update_color(color_id: str, patch: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if patch.get("exclusive_customer_id") is not None:
        ...
        upd["exclusive_source"] = {"kind": "manual", "at": now_iso()} if upd["exclusive_customer_id"] else {}
    if not upd:
...
_COLOR_PROJ = {"_id": 0, "id": 1, "code": 1, "name": 1, "hex": 1, "family": 1, "system": 1, "status": 1,
               ...
               "exclusive_source": 1, "use_requests": 1}
