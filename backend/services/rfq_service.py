...
async def _create_po_from_lines(supplier_id: str, lines: List[Dict[str, Any]],
    ...
    # R-7 (FASE F / PS-12) — PO hasil award RFQ tunduk gerbang produk sah yang sama dengan PO standar.
    from services import rnd_gate
    _prods = await db.products.find({"id": {"$in": [ln["product_id"] for ln in lines]}}, {"_id": 0}).to_list(len(lines))
    await rnd_gate.assert_orderable(_prods, entity_id=entity_id or "", where="PO dari RFQ")
