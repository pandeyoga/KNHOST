...
TURN_MAP: Dict[str, Dict[str, Dict[str, Any]]] = {
    "sales_orders": {
        "waiting_approval": {"roles": ("manager",), "title": "Setujui pesanan", "link": "approvals"},
        "waiting_stock":    {"roles": ("md", "warehouse_admin"), "title": "Stok kurang — atur pemenuhan", "link": "orders"},
        "approved":         {"roles": ("sales_admin",), "owner": True, "title": "Konfirmasi pesanan ke pelanggan", "link": "orders"},
        "confirmed":        {"roles": ("warehouse", "warehouse_admin"), "title": "Siapkan barang (picking)", "link": "operations"},
        "picked":           {"roles": ("warehouse", "warehouse_admin"), "title": "Kirim pesanan", "link": "operations"},
        "partially_shipped": {"roles": ("warehouse", "warehouse_admin"), "title": "Lanjutkan pengiriman sisa", "link": "operations"},
        "shipped":          {"roles": ("finance",), "title": "Terbitkan faktur & tagih", "link": "finance"},
    ...
    "purchase_orders": {
        ...
        "pending":   {"roles": ("warehouse", "warehouse_admin"), "title": "PO disetujui — siapkan penerimaan barang", "link": "operations"},
        "receiving": {"roles": ("warehouse", "warehouse_admin"), "title": "Lanjutkan penerimaan barang PO", "link": "operations"},
        "partial":   {"roles": ("md",), "owner": True, "title": "PO diterima sebagian — tindak lanjut ke pemasok", "link": "purchase-orders"},
        "completed": {"roles": ("finance",), "title": "PO lengkap — cocokkan tagihan pemasok", "link": "finance"},
        "rejected":  {"owner": True, "title": "PO ditolak", "link": "purchase-orders", "severity": "warning"},
    },
    "sales_returns": {
        "pending_approval": {"roles": ("manager",), "title": "Setujui retur penjualan", "link": "approvals"},
        "approved":         {"roles": ("warehouse", "warehouse_admin"), "title": "Terima barang retur & mulai inspeksi", "link": "returns"},
        "inspecting":       {"roles": ("warehouse_admin",), "title": "Selesaikan inspeksi retur", "link": "returns"},
        "inspected":        {"roles": ("finance", "sales_admin"), "title": "Putuskan penyelesaian retur (refund/kredit/nego)", "link": "returns"},
        "refund_settled":   {"owner": True, "title": "Retur selesai — refund", "link": "returns"},
    ...
    "purchase_returns": {
        "pending_approval": {"roles": ("manager",), "title": "Setujui retur pembelian", "link": "approvals"},
        "approved":         {"roles": ("warehouse", "warehouse_admin"), "title": "Kirim barang retur ke pemasok", "link": "purchase-returns"},
        "rejected":         {"owner": True, "title": "Retur pembelian ditolak", "link": "purchase-returns", "severity": "warning"},
    },
    "sample_requests": {
        "requested": {"roles": ("warehouse", "warehouse_admin"), "title": "Potong & siapkan sampel", "link": "operations"},
        "done":      {"owner": True, "title": "Sampel siap — serahkan ke pelanggan", "link": "sample-sales"},
        "cancelled": {"owner": True, "title": "Permintaan sampel dibatalkan", "link": "sample-sales", "severity": "warning"},
    },
}
ALIASES = {"sales_order": "sales_orders", "purchase_order": "purchase_orders", "sales_return": "sales_returns",
           "purchase_return": "purchase_returns", "sample_request": "sample_requests"}
...
async def notify_turn(coll: str, doc: Dict[str, Any], *, stale_days: int = 0) -> int:
    ...
    made = await create_addressed(
        roles=tuple(rule.get("roles") or ()), also_users=(owner,) if owner else (),
...
def turn_map_public() -> List[Dict[str, Any]]:
    return [{"collection": c, "status": s, "roles": list(r.get("roles") or ()), "owner": bool(r.get("owner")), "title": r["title"]}
