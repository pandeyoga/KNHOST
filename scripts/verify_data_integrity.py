...
async def layer2_db_invariants(db):
    ...
    # INV-DB-SORD (B4 · 2026-08-25): `special_orders.total_amount` WAJIB numerik.
    # Terukur: satu dokumen dengan nilai berupa TEKS (mis. "43.500.000" hasil impor)
    # membuat `float()` di jalur Papan PO Custom melempar ValueError → `GET
    # /api/home/admin` HTTP 500 → SELURUH Control Tower pemilik kosong. Jalur
    # bacanya sekarang bertahan-galat (`approval_backlog_service._as_float`), tetapi
    # dokumen bertipe salah tetap harus KETAHUAN — kalau tidak, nilai rupiahnya
    # dilaporkan 0 tanpa satu pun pesan.
    sord = await db.special_orders.find({}, {"_id": 0, "number": 1, "id": 1,
                                             "total_amount": 1}).to_list(3000)
    tipe_viol = [s.get("number", s.get("id")) for s in sord
                 if not isinstance(s.get("total_amount"), (int, float))
                 or isinstance(s.get("total_amount"), bool)]
    if tipe_viol:
        results["fail"] += 1
        line("FAIL", R, f"INV-DB-SORD: {len(tipe_viol)} special_orders.total_amount BUKAN angka",
             str(tipe_viol[:5]) + " → papan/pengingat akan melaporkan Rp 0")
    else:
        results["pass"] += 1
        line("PASS", G, f"INV-DB-SORD: {len(sord)} special_order — total_amount numerik")
...
async def layer_inspection_invariants(db):
    """FASE I (§3.4) — **DOKUMEN INSPEKSI BUKAN PINTU KE-3 UNTUK GRADE**.

    Angka cacat & grade hanya boleh lahir dari `qc_inspection_service` (4-point) yang
    memanggil `grade_service.set_roll_grade(source="qc_inspection")`. Dokumen
    `inspections.lines[]` adalah RINGKASAN + keputusan. Begitu dokumen boleh
    menghitung sendiri, akan ada dua angka untuk satu roll dan tidak ada cara memilih
    mana yang benar — itulah yang dijaga di sini, DUA ARAH:

    INV-QC-01  `points_snapshot` di dokumen == `inventory_rolls.inspection.points`.
    INV-QC-02a setiap baris dokumen ber-`grade_after ≠ grade_before` WAJIB punya
               `inventory_rolls.grade_history[]` yang cocok (roll itu,
               `source="qc_inspection"`).
    INV-QC-02b setiap `grade_history[]` ber-`source="qc_inspection"` WAJIB punya baris
               dokumen inspeksi yang menyebut roll itu. Arah ini yang menangkap
               "grade diubah lewat jalur lain lalu diberi label qc" — dan tanpa arah
               ini invarian hanya memeriksa dokumen yang ADA, bukan yang HILANG.

               **DIKECUALIKAN, tertulis** (§3.4): grade saat roll LAHIR (retur
               karantina lewat `return_service`), `quarantine_release`,
               `manager_override`, `migration` — semuanya BUKAN hasil inspeksi 4-point
               dan memang tidak punya dokumen SPK.
    INV-QC-03  baris yang DITAHAN (`hold`) wajib sejalan dengan roll-nya
               (`inventory_rolls.inspection.hold.held`), karena pagar putaway membaca
               yang di ROLL. Kalau dokumen bilang "ditahan" sementara roll tidak,
               barang menyimpang bisa masuk rak tanpa satu pun pesan.
    """
    print(f"\n{C}{B}L4-QC — Invarian Inspeksi & QC (FASE I \u00b7 anti pintu ke-3 grade){X}")
    EXEMPT_SOURCES = {"return_quarantine", "roll_created", "quarantine_release",
                      "manager_override", "migration", "initial"}
    EPS = 0.01

    docs = await db.inspections.find({}, {"_id": 0, "number": 1, "lines": 1}).to_list(5000)
    if not docs:
        results["pass"] += 1
        line("PASS", G, "INV-QC: belum ada dokumen inspeksi (skip — valid)")
        return

    roll_ids = {ln.get("roll_id") for d in docs for ln in (d.get("lines") or [])
                if ln.get("roll_id")}
    rolls = {r["id"]: r for r in await db.inventory_rolls.find(
        {"id": {"$in": list(roll_ids)}},
        {"_id": 0, "id": 1, "roll_no": 1, "inspection": 1, "grade_history": 1}
    ).to_list(20000)} if roll_ids else {}

    # ── INV-QC-01 + 02a + 03 (dokumen → roll) ────────────────────────────────
    beda_poin, grade_tanpa_jejak, tahanan_beda = [], [], []
    for d in docs:
        for ln in (d.get("lines") or []):
            rid = ln.get("roll_id")
            if not rid:
                continue                    # baris retur: hasilnya milik dokumen retur
            roll = rolls.get(rid)
            if not roll:
                continue                    # roll sudah tidak ada (dihapus/pindah PT)
            insp = roll.get("inspection") or {}
            snap = ln.get("points_snapshot")
            if snap not in (None, "") and insp.get("points") not in (None, ""):
                if abs(float(snap) - float(insp["points"])) > EPS:
                    beda_poin.append(f"{d.get('number')}/{ln.get('roll_no') or rid}"
                                     f"(dok {snap} vs roll {insp['points']})")
            if ln.get("grade_after") and ln.get("grade_after") != ln.get("grade_before"):
                cocok = [h for h in (roll.get("grade_history") or [])
                         if str(h.get("source") or "") == "qc_inspection"
                         # `grade_after` adalah nama kanonik yang ditulis
                         # `grade_service.set_roll_grade`. `to`/`grade` ditoleransi
                         # untuk jejak LAMA sebelum nama itu dibakukan — bukan
                         # kelonggaran, melainkan pengakuan bahwa data lama ada.
                         and str(h.get("grade_after") or h.get("to")
                                 or h.get("grade") or "") == ln["grade_after"]]
                if not cocok:
                    grade_tanpa_jejak.append(
                        f"{d.get('number')}/{ln.get('roll_no') or rid}\u2192{ln['grade_after']}")
            held_doc = bool(ln.get("hold"))
            held_roll = bool(((insp.get("hold") or {}).get("held")))
            if held_doc != held_roll:
                tahanan_beda.append(f"{d.get('number')}/{ln.get('roll_no') or rid}"
                                    f"(dok {held_doc} vs roll {held_roll})")

    for viol, label, ok_label in (
        (beda_poin, "INV-QC-01: poin dokumen \u2260 poin roll",
         "INV-QC-01: poin dokumen == poin roll (dokumen tetap RINGKASAN, bukan sumber)"),
        (grade_tanpa_jejak, "INV-QC-02a: grade berubah di dokumen tanpa jejak di roll",
         "INV-QC-02a: setiap perubahan grade di dokumen punya jejak `grade_history` roll"),
        (tahanan_beda, "INV-QC-03: tahanan dokumen \u2260 tahanan roll",
         "INV-QC-03: tahanan dokumen sejalan dengan roll (pagar putaway membaca roll)"),
    ):
        if viol:
            results["fail"] += 1
            line("FAIL", R, f"{label}: {len(viol)} baris", str(viol[:5]))
        else:
            results["pass"] += 1
            line("PASS", G, ok_label)

    # ── INV-QC-02b (roll → dokumen): arah yang menangkap dokumen yang HILANG ──
    doc_rolls = {ln.get("roll_id") for d in docs for ln in (d.get("lines") or [])
                 if ln.get("roll_id")}
    yatim = []
    async for r in db.inventory_rolls.find(
            {"grade_history.source": "qc_inspection"},
            {"_id": 0, "id": 1, "roll_no": 1, "grade_history": 1}):
        jejak = [h for h in (r.get("grade_history") or [])
                 if str(h.get("source") or "") == "qc_inspection"]
        if jejak and r["id"] not in doc_rolls:
            yatim.append(f"{r.get('roll_no') or r['id']}({len(jejak)} jejak)")
    if yatim:
        results["fail"] += 1
        line("FAIL", R, f"INV-QC-02b: {len(yatim)} roll ber-grade_history "
                        f"`qc_inspection` tanpa baris dokumen inspeksi", str(yatim[:5]))
    else:
        results["pass"] += 1
        line("PASS", G, f"INV-QC-02b: seluruh perubahan grade ber-source "
                        f"`qc_inspection` punya dokumennya "
                        f"(dikecualikan tertulis: {', '.join(sorted(EXEMPT_SOURCES))})")
...
async def layer_rnd_invariants(db):
    ...
    INV-RND-02  Round yang sudah ditutup (ada `result`) WAJIB punya lampiran bukti +
                ...
                dan `round_no` berurut tanpa lompatan **per (supplier × jenis)** — sejak
                FASE S satu permintaan boleh menempuh beberapa jenis sampling sekaligus.
    ...
    n_rounds = n_closed = 0
    for s in samples:
        # FASE S — deret nomor round dihitung per (SUPPLIER × JENIS), bukan per
        # supplier. Satu permintaan boleh menempuh labdip DAN handfeel pada supplier
        # yang sama; itu dua rangkaian pekerjaan yang berbeda, masing-masing dengan
        # rnd 1, 2, 3 sendiri (`rnd_sample_service.open_round` menomori begitu, dan
        # kuota `rnd.max_rounds` pun dihitung per jenis). Aturan lama "satu deret per
        # supplier" menuduh data yang SEHAT: gate ini memerah pada data demo dengan
        # pesan "nomor round tidak berurut [1, 1]" — dua round jenis berbeda.
        kinds = [str(v).strip().lower() for v in (s.get("sample_types") or [])
                 if str(v or "").strip()]
        if not kinds and s.get("sample_type"):     # dokumen sebelum migrasi FASE S
            kinds = [str(s.get("sample_type")).strip().lower()]
        per_key = {}
        for r in (s.get("rounds") or []):
            n_rounds += 1
            # Cermin `rnd_sample_service._type_of_round`: round lama tanpa
            # `type_code` diikat ke jenis pertama dokumennya (satu definisi).
            tcode = (str(r.get("type_code") or "").strip().lower()
                     or (kinds[0] if kinds else ""))
            per_key.setdefault((r.get("supplier_id"), tcode), []) \
                   .append(int(r.get("round_no") or 0))
            if not r.get("result"):
                continue
            n_closed += 1
        ...
        for (sup, tcode), nos in per_key.items():
            if sorted(nos) != list(range(1, len(nos) + 1)):
                bad2.append(f"{s.get('number')}/{sup}/jenis {tcode or '—'}: "
                            f"nomor round tidak berurut {sorted(nos)}")
    if bad2:
        results["fail"] += 1
        ...
        line("PASS", G, f"INV-RND-02: {n_closed} dari {n_rounds} round tertutup — semuanya "
                        "ber-lampiran & ber-catatan; nomor round berurut per "
                        "(supplier × jenis sampling)")
    ...
    # ── INV-RND-06
    # FASE S — "wajib desain" kini DATA di master jenis (`requires_design`), dan satu
    # permintaan boleh menempuh beberapa jenis. Yang diperiksa: dokumen yang memuat
    # jenis ber-`requires_design` WAJIB merujuk desain nyata. Daftar jenisnya dibaca
    # dari master (dengan cadangan benih) supaya jenis baru yang menuntut desain ikut
    # terjaga tanpa menyunting alat ini.
    designs = {d["id"] for d in await db.design_gallery.find({}, {"_id": 0, "id": 1}).to_list(5000)}
    need_design = {str(r.get("code") or r.get("value") or "").strip().lower()
                   for r in await db.sample_types.find(
                       {"requires_design": True}, {"_id": 0, "code": 1}).to_list(200)}
    if not need_design:
        try:
            import domain_registry as _dr
            need_design = {str(r.get("value") or "").lower()
                           for r in _dr.enum_items("sample_type")
                           if r.get("requires_design")}
        except Exception:  # noqa: BLE001 — alat ukur tidak boleh mati karena import
            need_design = {"proofing"}

    def _types(s):
        out = [str(v).strip().lower() for v in (s.get("sample_types") or [])
               if str(v or "").strip()]
        if not out and s.get("sample_type"):
            out = [str(s["sample_type"]).strip().lower()]
        return out

    proofs = [s for s in samples if need_design & set(_types(s))]
    bad6 = [s.get("number") for s in proofs
            if not s.get("design_id") or s.get("design_id") not in designs]
    if bad6:
        results["fail"] += 1
        line("FAIL", R, f"INV-RND-06: {len(bad6)} permintaan ber-jenis wajib-desain "
                        f"({', '.join(sorted(need_design))}) tanpa desain sah",
             str(bad6[:5]))
    else:
        results["pass"] += 1
        line("PASS", G, f"INV-RND-06: {len(proofs)} permintaan ber-jenis wajib-desain "
                        "\u2014 semuanya merujuk desain nyata di master desain")
...
async def layer_bank_invariants(db):
    ...
    # Titipan yang SUDAH punya kasus keuangan terbuka (FASE G-9) berarti tindak lanjutnya
    # sedang berjalan — bukan "menganggur".
    handled = {c.get("source", {}).get("id") for c in await db.finance_cases.find(
        {"source.kind": "bank_holding", "status": {"$nin": ["resolved", "closed", "cancelled"]}},
        {"_id": 0, "source.id": 1}).to_list(5000)}
    for l in holding:
        if l.get("id") in handled:
            continue
        try:
...
async def layer_gl_audit_2026_09_invariants(db):
    ...
    # T-05 (audit 2026-09) — KEB-PDPT tahap 2 (iter298/299, pro-rata) mengakui pendapatan
    # PER SURAT JALAN (`shipment_revenue`, ref.order_id); pemeriksa ini masih hanya melihat
    # `sales_order` sehingga menuduh palsu 3 SO demo. Kedua jalur dihitung.
    ...
    rev_ids |= {(j.get("ref") or {}).get("order_id") for j in await db.journal_entries.find(
        {"source_type": "shipment_revenue", "status": {"$ne": "void"}}, {"_id": 0, "ref.order_id": 1}).to_list(50000)}
    missing = []
    n_rev = 0
...
def _layer_registry():
    return [
        ...
        ("inspection",  layer_inspection_invariants,      True,  ("INV-QC",)),
