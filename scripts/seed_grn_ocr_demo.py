"""Demo GRN Fase 4: satu kedatangan KSC/PO-00014 yang dibaca dengan model TIRUAN `mock-sj_doubt` (angka ragu + beda
...
def main() -> None:
    ...
    if db.goods_receipts.find_one({"dn.number": "SJ-MOCK-DOUBT", "status": {"$nin": ["cancelled", "rejected"]}}):
        print("Demo sudah ada — lewati.")
        return
    cfg(s, {"receiving.ocr_enabled": True, "receiving.ocr_model_primary": "mock-sj_doubt",
            "receiving.ocr_model_second": "mock-sj_doubt_second"})
    try:
        g = s.post(f"{BASE}/goods-receipts", json={"partner_type": "supplier", "partner_id": po["supplier_id"],
                                                   "warehouse_id": t["warehouse_id"], "po_ids": [t["po_id"]]}).json()
        ...
        print(f"{g['number']} → {g['status']} · baris {len(g['lines'])} · runs {len(g['extraction']['runs'])}")
    finally:
