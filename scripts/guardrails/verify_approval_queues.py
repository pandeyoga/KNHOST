...
DOOR_QUEUE: Dict[str, str] = {
    ...
    # ── FASE D (2026-08-20) — PERMINTAAN DESAIN. Pintunya ACC / minta revisi; yang
    # menunggu = `design_requests.status == "delivered"` (artwork sudah diserahkan,
    # atasan belum memutuskan). Antreannya sudah ada di `QUEUES`; tanpa dua baris ini
    # gate memerah dengan benar karena pintu dan antrean tidak pernah dijodohkan.
    # Perhatikan: ini BUKAN kembaran `design_gallery.approve` — galeri menilai
    # ARTWORK-nya (berkas/versi), permintaan desain memutuskan PEKERJAAN-nya
    # (penugasan & tenggat). Dua antrean berbeda dengan sengaja; `check_disjoint`
    # membuktikan tak ada satu dokumen pun yang dihitung dua kali.
    "design_requests.py::/api/design-requests/{req_id}/approve": "design_request",
    "design_requests.py::/api/design-requests/{req_id}/reject": "design_request",
}
...
DOOR_EXEMPT: Dict[str, str] = {
    ...
    "color_library.py::/api/color-library/use-requests/{req_id}/approve":
        "Izin pakai warna eksklusif disimpan sebagai `color_library.use_requests[]` "
        "(master BERSAMA tanpa entity_id; satu warna bisa memuat beberapa permintaan) sehingga "
        "belum cocok dengan model antrean satu-dokumen-satu-item ber-entity_id. Antreannya: panel "
        "'Izin pakai warna eksklusif' di Pustaka Warna → Warna Pelanggan (lencana jumlah menunggu) "
        "+ notifikasi ke manajer saat diajukan.",
    "color_library.py::/api/color-library/use-requests/{req_id}/reject":
        "Idem pintu approve — antrean di panel izin Pustaka Warna → Warna Pelanggan.",
