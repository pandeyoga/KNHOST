...
TERGATED_DI_KODE = {
    ...
    # ── FASE D (2026-08-20) — layar Permintaan Desain dilihat 4 peran ────────────
    # Catatan penting soal DUA baris di bawah: keduanya pagar NYATA di layar yang
    # sama, dan sengaja TIDAK ditutup dengan melebarkan izin desainer. Dua tuduhan
    # lain pada peran ini (`/suppliers` & `/warehouses` di layar Desain & Pattern +
    # Galeri Desain) TIDAK muncul di daftar ini karena diperbaiki di AKARNYA:
    # endpointnya dulu menumpang di modul bersama `features/rnd/rndApi.js` padahal
    # kedua layar itu tak pernah memanggilnya (`listWarehouses` bahkan nol pemakai).
    # Pembebasan hanya sah bila pagarnya memang ada; kalau sebabnya "endpoint salah
    # tempat", obatnya memindahkannya — bukan menambah baris di sini.
    ("design-requests", "/sales-orders"):
        "features/design/DesignRequestsView.jsx — daftar pesanan hanya diambil di "
        "dalam `if (!canCreate) return;` (sumber permintaan baru); desainer tidak "
        "boleh membuat permintaan, jadi <DesignRequestCreateModal> tak pernah dirender",
    # ── Audit peran 2026-09-22 — 8 pagar nyata didaftarkan; 1 temuan (sales →
    # /design-requests/meta di form Pesanan Khusus) diperbaiki di AKAR (izin endpoint
    # menerima `order.create`), jadi tidak ada di daftar ini.
    ("md-products", "/suppliers"):
        "features/catalog/CatalogWorkspace.jsx — tombol 'Data SKU & impor' (legacyView → "
        "AdminView/ProductMasterForm → hooks/useCatalogMasters) hanya dirender bila caps.update",
    ("purchasing", "/finance/budget-keys"):
        "features/admin/PurchaseOrderManagement.jsx — menu Buat (POCreateForm → POBudgetPanel) "
        "disembunyikan bila user.role === 'finance'",
    ("purchasing", "/makloon-orders/claims"):
        "features/admin/PurchaseOrderManagement.jsx — MakloonOrderCreateModal (pengimpor "
        "makloonApi di layar ini; hanya memakai fetchStages) lahir dari menu Buat yang "
        "disembunyikan untuk finance",
    ("purchasing", "/makloon-orders/claims/stats"):
        "features/admin/PurchaseOrderManagement.jsx — idem (MakloonOrderCreateModal)",
    ("purchasing", "/makloon-partners/scorecard"):
        "features/admin/PurchaseOrderManagement.jsx — idem (MakloonOrderCreateModal)",
    ("purchasing", "/warehouses"):
        "features/admin/PurchaseOrderManagement.jsx — fetchMasterData() hanya bila canCreate "
        "(role ≠ finance); MakloonOrderCreateModal lahir dari menu Buat yang sama",
    ("purchasing", "/supplier-contracts"):
        "features/admin/PurchaseOrderManagement.jsx — makloonApi (kontrak supplier) hanya "
        "diimpor MakloonOrderCreateModal (menu Buat, disembunyikan untuk finance)",
    ("purchasing", "/supplier-contracts/policy"):
        "features/admin/PurchaseOrderManagement.jsx — idem (makloonApi)",
    ("purchasing", "/supplier-contracts/stats"):
        "features/admin/PurchaseOrderManagement.jsx — idem (makloonApi)",
    ("purchasing", "/rfid/labels"):
        "features/admin/po/PODetailPanel.jsx — <PrintPoLabelsButton> hanya dirender bila "
        "can(perms,'wms','view') (cetak label roll = urusan gudang)",
    ("purchasing", "/makloons"):
        "features/admin/PurchaseOrderManagement.jsx — components/MakloonSelect hanya hidup di "
        "MakloonOrderCreateModal (menu Buat disembunyikan untuk finance)",
    ("purchasing", "/process-recipes"):
        "features/admin/PurchaseOrderManagement.jsx — idem (MakloonOrderCreateModal)",
    ("purchasing", "/process-stages"):
        "features/admin/PurchaseOrderManagement.jsx — idem (makloonApi.fetchStages dari MakloonOrderCreateModal)",
    ("*", "/md/desk"):
        "features/sales_admin/workDeskApi.js — modul API bersama; mdDesk hanya dipanggil "
        "features/desks/RoleDesk.jsx desk='md' (layar md-desk, menu peran md)",
    ("*", "/warehouse-admin/desk"):
        "features/sales_admin/workDeskApi.js — modul API bersama; warehouseAdminDesk hanya "
        "dipanggil features/desks/RoleDesk.jsx desk='warehouse_admin' (layar warehouse-admin-desk)",
    ("design-requests", "/design/reports/by-designer"):
        "features/design/DesignRequestsView.jsx — tab 'Rapor Desainer' hanya dirender "
        "bila canReport (admin/manager: rapor menilai ORANG). Desainer mendapat tab "
        "'Rapor Saya' → GET /design/reports/mine (angka dirinya, tanpa nama rekan)",
}
