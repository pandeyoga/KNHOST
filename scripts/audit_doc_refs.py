...
async def audit_health(strict: bool) -> None:
    ...
    # Tautan menggantung dihitung oleh `doc_refs_service.dangling_refs()` — SATU
    # definisi yang dipakai bersama `gate_residue.py --check` (INV-GATE-01) dan
    # `verify_ref_unlink.py` (INV-REF-04). Salinan lokal di skrip ini sudah pernah
    # membuat gate & POC memeriksa hal yang "hampir sama", dan celah itulah yang
    # meloloskan residu POC E-7 selama berbulan-bulan.
    # Laporannya menyebut **NOMOR DOKUMEN** yang masih hidup, bukan hanya id acak.
    # Terukur 2026-08-23: laporan lama berbunyi "Tautan menggantung: 1 temuan" dengan
    # sepasang id (`inspection:ins_8f2… → purchase_order:po_a91…`) — dan karena dokumen
    # tujuannya memang sudah TIADA, tidak ada satu pun petunjuk POC mana penyebabnya.
    # Diagnosisnya menelan dua kali `gate --full` + pemantau khusus. `dangling_refs()`
    # sudah menghitung `from_number`/`to_number`; versi lama hanya membuangnya.
    dangling = [f"{g['from_number']} ({g['from']}) —{g['rel'] or 'ref'}→ "
                f"{g['to_number'] or g['to']} [HILANG]"
                for g in await refs.dangling_refs(limit=50)]
