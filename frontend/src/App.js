...
function App() {
  ...
  const {
    ...
    caseFocus, clearCaseFocus, logisticsFocus, clearLogisticsFocus,
  } = useDeepLinks({
    setActiveNavId, setActiveView, setSidebarOpen, ready: Boolean(user),
    // FB-02 — dari Logistik → detail Pesanan (reuse jalur focusDoc EPIC6).
    onOpenOrder: (orderId) => openDocument({ view: "orders", nav_id: "sales-orders", focus_type: "sales_order", focus_id: orderId }),
  });
  ...
  const openDocument = (link) => {
    ...
    if (link.focus_id) setFocusDoc({ focus_type: link.focus_type, focus_id: link.focus_id, number: link.number || "", tab: link.tab || "" });
  ...
  // Master/R&D are responsive pages, not redirects to the mobile approval inbox.
  // Keep an explicitly selected/deep-linked page when the viewport changes.
  const responsiveWorkspace = ['md-products', 'product-templates', 'rnd-specs'].includes(activeView);
  const goDesktop = () => { localStorage.setItem("kn_force_desktop", "1"); setForceDesktop(true); };
  if (wantMobile && !responsiveWorkspace && user.role !== "sales") {
    ...
    return <MobileApp user={user} onLogout={logout} entities={entities} selectedEntity={selectedEntity}
      unreadCount={unreadCount} onForceDesktop={goDesktop} onNavigate={handleNavSelect} />;
  }
  if (user.role === "sales" && wantMobile && !responsiveWorkspace) {
    return (
      <MobileSalesApp
        user={user}
        token={token}
      ...
      <div className="app-main">
        ...
        <main id="main-content" className="mx-auto w-full max-w-[1600px] px-4 py-4 md:px-5 md:py-5">
          ...
          <AppViewRouter
            ...
            logisticsFocus={logisticsFocus}
            onLogisticsFocusConsumed={clearLogisticsFocus}
