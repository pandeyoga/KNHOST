...
const SamplePriceMaster = lazy(() => import("./features/samples/SamplePriceMaster"));
...
const CatalogWorkspace = lazy(() => import("./features/catalog/CatalogWorkspace"));
...
const LogisticsView = lazy(() => import("./features/logistics/LogisticsView"));   // FB-02
...
// REVISI SAMPEL — meja & daftar pesanan sampel
const SampleAdminDesk = lazy(() => import("./features/samples/SampleAdminDesk"));
const SampleOrdersView = lazy(() => import("./features/samples/SampleOrdersView"));
...
const SampleTypeGalleryView = lazy(() => import("./features/rnd/SampleTypeGalleryView"));
...
const ContentCalendar = lazy(() => import("./features/marketing/ContentCalendar"));
const MktCampaigns = lazy(() => import("./features/marketing/Campaigns"));
const MarketingAnalytics = lazy(() => import("./features/marketing/MarketingAnalytics"));
const SocialAccounts = lazy(() => import("./features/marketing/SocialAccounts"));
...
export default function AppViewRouter(props) {
  const {
    ...
    logisticsFocus, onLogisticsFocusConsumed,
    lastDocument, lastLabel,
  } = props;
  const [catalogFocus, setCatalogFocus] = useState(null);
  useEffect(() => {
    const handler = e => { setCatalogFocus({ ...e.detail, nonce: Date.now() }); onNavSelect('products-pricing', 'md-products'); };
    window.addEventListener('kn-open-catalog', handler);
    return () => window.removeEventListener('kn-open-catalog', handler);
  }, [onNavSelect]);
  useEffect(() => {
    const handler = () => actions.loadAll();
    window.addEventListener('kn-catalog-changed', handler);
    return () => window.removeEventListener('kn-catalog-changed', handler);
  }, [actions]);
  ...
  return (
    <div className="mt-4 md:mt-5">
      ...
      {activeView === "md-products" && <CatalogWorkspace currentUser={user} selectedEntity={selectedEntity} focus={catalogFocus} legacyView={<AdminView {...adminProps} only={["products"]} />} />}
      ...
      {/* REVISI SAMPEL — Meja Admin Sampel + daftar Pesanan Sampel (SMP-) terpisah dari SO biasa */}
      {activeView === "sample-admin-desk" && <SampleAdminDesk currentUser={user} selectedEntity={selectedEntity} onOpenDocument={openDocument} />}
      {activeView === "sample-orders" && <SampleOrdersView user={user} selectedEntity={selectedEntity} focusDoc={focusDoc} onClearFocus={() => setFocusDoc(null)} />}
      {activeView === "returns" && (
      ...
      {activeView === "product-templates" && <CatalogWorkspace currentUser={user} selectedEntity={selectedEntity} focus={catalogFocus} legacyView={<AdminView {...adminProps} only={["products"]} />} />}
      ...
      {["rnd-labdip", "rnd-handfeel", "rnd-proofing"].includes(activeView) && (
        <SampleTypeGalleryView key={activeView} code={activeView.replace("rnd-", "")} currentUser={user} selectedEntity={selectedEntity} />
      )}
      {activeView === "rnd-designs" && (
        <RndDesignsView currentUser={user} selectedEntity={selectedEntity}
          focus={rndFocus?.view === "rnd-designs" ? rndFocus : null}
          onFocusConsumed={onRndFocusConsumed} />
      )}
      ...
      {activeView === "mkt-calendar" && <ContentCalendar currentUser={user} selectedEntity={selectedEntity} />}
      {activeView === "mkt-campaigns" && <MktCampaigns currentUser={user} />}
      {activeView === "mkt-analytics" && <MarketingAnalytics currentUser={user} selectedEntity={selectedEntity} />}
      {activeView === "mkt-accounts" && <SocialAccounts currentUser={user} selectedEntity={selectedEntity} />}
