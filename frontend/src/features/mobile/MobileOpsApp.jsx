...
export default function MobileOpsApp(props) {
  ...
  if (['admin', 'manager', 'md'].includes(props.user?.role)) tabs.splice(4, 0, {
    id: 'catalog', label: 'Produk', icon: ClipboardList,
    render: () => <div className="space-y-3 p-5"><button data-testid="mobile-open-catalog" className="primary-button w-full justify-center" onClick={() => props.onNavigate?.('products-pricing', 'md-products')}>Produk & varian</button><button data-testid="mobile-open-rnd" className="secondary-button w-full justify-center" onClick={() => props.onNavigate?.('rnd-hub', 'rnd-specs')}>Spesifikasi R&D</button></div>,
  });
