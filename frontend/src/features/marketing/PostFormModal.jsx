...
export default function PostFormModal({ meta, campaigns = [], initial = {}, post = null, currentUser, onClose, onSaved }) {
  ...
  const applyTemplate = (t) => {
    setTemplateId(t?.id || "");
    if (!t) return;
    setF((s) => ({ ...s, title: t.title || s.title, caption: t.caption || "", hashtags: (t.hashtags || []).map((h) => `#${h}`).join(" "),
      platforms: t.platforms?.length ? t.platforms : s.platforms, content_type: t.content_type || s.content_type, cta: t.cta || "", notes: t.notes || "" }));
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!post && templateId) mktApi.useTemplate(templateId).catch(() => {});
      onSaved?.(saved);
  ...
  return (
    <FormModal open title={post ? `Ubah konten · ${post.title}` : "Konten baru"} subtitle="Rencanakan post: apa, di mana, kapan, siapa yang memposting." icon={Megaphone} size="lg"
      ...
      <div className="grid gap-3">
        {!post && <TemplatePicker value={templateId} onApply={applyTemplate} currentUser={currentUser} />}
