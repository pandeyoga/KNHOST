...
import { Copy, FileText, Image as ImageIcon, LayoutTemplate, Loader2, Megaphone, Pencil, Trash2, Upload, X } from "lucide-react";
import FormModal from "../../components/FormModal";
import { askReason, askConfirm, askText } from "@/services/confirmService";
...
export default function PostDetailModal({ postId, meta, campaigns, currentUser, onClose, onChanged, onOpenPost }) {
  ...
  const saveTemplate = async () => {
    const name = await askText({ title: "Simpan sebagai template", reasonLabel: "Nama template", message: "Caption, hashtag, platform, jenis konten, CTA & catatan disimpan untuk dipakai ulang.", testId: "mkt-template-name" });
    if (!name) return;
    try {
      await mktApi.createTemplate({ name, title: post.title, caption: post.caption, hashtags: post.hashtags, platforms: post.platforms, content_type: post.content_type, cta: post.cta, notes: post.notes, source_post_id: post.id });
      setInfo(`Template "${name}" tersimpan — pilih di "Konten baru".`);
    ...
    <>
      <FormModal open title={post.title} subtitle={`${post.content_type} · ${post.pic_name ? `PIC ${post.pic_name}` : "tanpa PIC"} · dibuat ${post.created_by}`} icon={Megaphone} size="lg" onClose={onClose} testId="mkt-post-detail"
        footer={<div className="flex w-full flex-wrap items-center gap-2">
          ...
          {canEdit && <button type="button" className="secondary-button !py-1 text-[11px]" onClick={() => setDup(true)} data-testid="mkt-duplicate-post"><Copy size={12} /> Duplikat</button>}
          {canEdit && <button type="button" className="secondary-button !py-1 text-[11px]" onClick={saveTemplate} data-testid="mkt-save-template"><LayoutTemplate size={12} /> Simpan sebagai template</button>}
        ...
        <div className="grid gap-3 text-[12px]">
          ...
          {info && <p className="text-[11px] text-[#1B7F4B]" data-testid="mkt-detail-info">{info}</p>}
      ...
      {dup && <DuplicatePostModal post={post} onClose={() => setDup(false)} onDone={(p) => { setDup(false); onChanged?.(); onOpenPost?.(p.id); }} />}
      {edit && <PostFormModal meta={meta} campaigns={campaigns} post={post} currentUser={currentUser} onClose={() => setEdit(false)} onSaved={(p) => { setPost(p); setEdit(false); onChanged?.(); }} />}
    </>
  );
}
