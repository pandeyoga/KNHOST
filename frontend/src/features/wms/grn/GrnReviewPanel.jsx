...
import { GrnDraftActions, GrnExtractionNotice, LineDoubtRow, writtenQty } from "./GrnOcrBits";
...
function LineRow({ grn, ln, targets, run }) {
  ...
  const tone = ln.decision === "reject_line" ? "bg-[#F2F2F7] text-[#6B6B73]" : doubt || uom === "uom_unknown" || (!ln.target && !ln.is_non_stock) ? "bg-[#FDECEC] text-[#B4231F]" : ln.match?.status === "manual" || ln.match?.status === "exact" ? "bg-[#E7F6F3] text-[#0F766E]" : "bg-[#FFF6E5] text-[#B26A00]";
  const status = ln.decision === "reject_line" ? "Ditolak" : ln.is_non_stock ? "Non-stok" : !ln.target ? "Pilih target" : doubt ? "Angka ragu" : uom === "uom_unknown" ? "Satuan?" : "Siap";
  ...
  return (<>
    <tr data-testid={`grn-line-row-${ln.line_no}`} className="border-t border-[#EFF0F2] text-[11px]">
      ...
      <td className="px-2 font-mono text-[10.5px] text-[#6B6B73]" data-testid={`grn-line-written-${ln.line_no}`}>{writtenQty(ln) || "-"}</td>
      <td className="px-2 font-mono tabular-nums" data-testid={`grn-line-qty-${ln.line_no}`}>{d.qty ?? "-"}</td>
    ...
    <LineDoubtRow ln={ln} colSpan={13} onPick={(declared) => patch({ declared })} />
  </>);
}
