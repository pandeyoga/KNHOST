...
export default function InboundTaskPanel({
  ...
  const [mode, setMode] = useState((task.qty_rolls_scanned || 0) > 0 || !(task.scan_log || []).some((s) => s.scan_type === "receive") ? "label" : "manual");
          ...
          <>
            <div className="flex items-center gap-1 rounded-lg bg-[#F2F2F7] p-0.5 text-[10.5px] font-semibold" data-testid="inbound-mode-toggle">
              <button type="button" onClick={() => setMode("label")} data-testid="inbound-mode-label"
                className={`flex flex-1 items-center justify-center gap-1 rounded-md px-2 py-1 ${mode === "label" ? "bg-white text-[#0058CC] shadow-sm" : "text-[#6B6B73]"}`}>
                <ScanLine size={12} /> Scan Label Supplier
              </button>
              <button type="button" onClick={() => setMode("manual")} data-testid="inbound-mode-manual"
                className={`flex flex-1 items-center justify-center gap-1 rounded-md px-2 py-1 ${mode === "manual" ? "bg-white text-[#0058CC] shadow-sm" : "text-[#6B6B73]"}`}>
                <Keyboard size={12} /> Input Manual
              </button>
            </div>

            {mode === "label" && (
              <ScanLabelPanel task={task} onTaskUpdated={onTaskUpdated} onEscalate={onEscalate} />
            )}
            ...
            <div className="flex gap-2">
              ...
              {canComplete && (
                <button onClick={onComplete} disabled={submitting}
                  ...
                  <TrendingUp size={13} /> {(task.qty_rolls_scanned || 0) > 0 ? "Selesaikan Penerimaan" : "Complete"}
                </button>
              )}
            ...
            {mode === "manual" && (task.scan_log || []).length > 0 && (
