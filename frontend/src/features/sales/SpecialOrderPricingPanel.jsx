  ...
  return (
    <section className="section-card !p-3" data-testid="od-pricing">
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        {canLock && (<>
          <KNSelect className="input !w-56 !py-1 text-[11px]" value={warehouseId} onValueChange={setWarehouseId} data-testid="od-price-warehouse" placeholder="Gudang tujuan PO"
            options={warehouses.length === 0 ? [{ value: "", label: "gudang utama (bawaan)" }] : warehouses.map((w) => ({ value: w.id, label: `${w.code ? `${w.code} · ` : ""}${w.name}` }))} />
